...
async function main(): Promise<void> {
  ...
  const p95BudgetPct = parsePctEnv('PIPELINE_BENCH_MAX_REGRESSION_P95_PCT', PIPELINE_BENCH_CI_MAX_REGRESSION.p95Pct)
  const p99BudgetPct = parsePctEnv('PIPELINE_BENCH_MAX_REGRESSION_P99_PCT', PIPELINE_BENCH_CI_MAX_REGRESSION.p99Pct)

  const baseline = await readBaseline(baselinePath)
  const currentPath = resolve(process.cwd(), CI_OUTPUT_PATH)

  const args = [
    'run', 'src/backtest/run-pipeline-benchmark.ts',
    '--coins', PIPELINE_BENCH_CI_COINS.join(','),
    '--tfs', PIPELINE_BENCH_CI_TIMEFRAMES.join(','),
    '--bars', String(PIPELINE_BENCH_CI_BARS_PER_SERIES),
    '--warmup', String(PIPELINE_BENCH_CI_WARMUP_RUNS),
    '--runs', String(PIPELINE_BENCH_CI_MEASURED_RUNS),
    '--trim', String(PIPELINE_BENCH_CI_TRIM_RATIO),
    '--out', currentPath,
  ]
  ...
  console.log(` Budget:   p95<=${(p95BudgetPct * 100).toFixed(1)}% p99<=${(p99BudgetPct * 100).toFixed(1)}% regression`)
  console.log('-'.repeat(70))
  ...
  const baselineMetrics = mode === 'robust' ? baseline.metrics.robust : baseline.metrics.raw
  const currentMetrics = selectBudgetMetrics(currentResult, mode)
  const regression = evaluateRegression(
    baselineMetrics,
    currentMetrics,
    { p95Pct: p95BudgetPct, p99Pct: p99BudgetPct },
  )
...
function printSide(label: 'p95' | 'p99', side: ReturnType<typeof evaluateRegression>['p95']): void {
