...
export function evaluateHeartbeat(args: {
  ...
  if (isAlive(record.pid)) {
    return { action: 'cancel', reason: 'stale-but-alive', pid: record.pid, ageMs }
  }
...
async function main(): Promise<void> {
  if (!isBbWatchdogEnabled()) {
    log.error('bb-watchdog', 'refusing to start: BB watchdog is only enabled when ACTIVE_EXCHANGE=BB AND PAPER_TRADE=false')
    process.exit(2)
  }
  ...
  try {
    ...
    log.error('bb-watchdog', `Bybit init failed: ${err instanceof Error ? err.message : err}`)
    process.exit(3)
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (result.success) {
        log.error('bb-watchdog', 'cancelAllOpenOrders OK — exiting non-zero for supervisor restart')
      } else {
        log.error('bb-watchdog', `cancelAllOpenOrders failed: ${result.error} — exiting anyway`)
      }
    } catch (err) {
      log.error('bb-watchdog', `cancelAllOpenOrders exception: ${err instanceof Error ? err.message : err}`)
    }
    process.exit(1)
  }
}
...
if (import.meta.main) {
  main().catch(err => {
    log.error('bb-watchdog', `fatal: ${err instanceof Error ? err.message : err}`)
    process.exit(4)
  })
}
