...
const DEFAULT_LIBRARY_ROOT = '/Users/luongthebinh/Downloads/Personal/charting_library-29.4.0'
...
async function main(): Promise<void> {
  ...
  const chartingTarget = path.join(root, 'dashboard', 'public', 'charting_library')
  const datafeedsTarget = path.join(root, 'dashboard', 'public', 'datafeeds')

  const sourceReady = await pathExists(chartingSource) && await pathExists(datafeedsSource)
  if (!sourceReady) {
    const targetReady = await pathExists(chartingTarget) && await pathExists(datafeedsTarget)
    if (targetReady) {
      console.log(`TradingView assets already present in dashboard/public; source path missing: ${sourceRoot}`)
      return
    }
    throw new Error(
