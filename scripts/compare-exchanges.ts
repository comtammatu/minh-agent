...
const signalTfs = (parseListArg("--tf") ?? [
  ...SIGNAL_TIMEFRAMES,
]) as CandleInterval[];
...
async function backfillExchange(
  ...
  const results = await feed.backfill(coins, (coin, interval, candles) => {
  ...
  console.log(
    `[${now()}] ✅ ${label}: ${totalCandles} candles loaded | ${fullyReady}/${coins.length} fully ready`,
  );
}
  ...
  for (const coin of coins) {
    for (const tf of signalTfs) {
      ...
      results.push({
        ...
        avgCloseDiffPct:
          matched.length > 0 ? totalCloseDiff / matched.length : 0,
  ...
  const onSetup = (setup: ActiveSetup) => {
    collected.push({
      ...
      signal: {
        ...
        ...(setup.confluenceGrade !== undefined
          ? { confluenceGrade: setup.confluenceGrade }
          : {}),
        ...(setup.confluenceCount !== undefined
...
async function startLiveComparison(
  ...
  const liveSignals = new Map<
    string,
    { hl?: CollectedSignal; bb?: CollectedSignal; ts: number }
  >();
  ...
  function onClosedCandle(
    ...
    if (signal) {
      ...
      if (entry.hl && entry.bb) {
        const diff = pct(
          entry.hl.signal.entryPrice,
          entry.bb.signal.entryPrice,
        );
        const confDiff =
  ...
  console.log(
    `[${now()}] ✅ WS subscribed — waiting for closed candles (Ctrl+C to stop)\n`,
  );
...
function printCandleReport(comparisons: CandleComparison[]): void {
  ...
  for (const [coin, comps] of byCoin) {
    console.log(`\n  ${coin}:`);
    console.log(
      `  ${"TF".padEnd(6)} ${"HL".padStart(7)} ${"BB".padStart(7)} ${"Match".padStart(7)} ${"ΔClose%".padStart(9)} ${"MaxΔ%".padStart(9)} ${"VolRatio".padStart(10)}`,
    );
  ...
  const avgDiff =
    comparisons.length > 0
      ? comparisons.reduce((s, c) => s + c.avgCloseDiffPct, 0) /
        comparisons.length
      : 0;
...
function printSignalReport(matches: SignalMatch[]): void {
  ...
  if (bothExchanges.length > 0) {
    ...
    console.log(
      `  ${"Coin".padEnd(8)} ${"TF".padEnd(5)} ${"Engine".padEnd(8)} ${"Side".padEnd(6)} ${"HL Entry".padStart(12)} ${"BB Entry".padStart(12)} ${"ΔEntry%".padStart(9)} ${"HL Conf".padStart(8)} ${"BB Conf".padStart(8)} ${"ΔConf".padStart(7)}`,
    );
  ...
  if (bothExchanges.length > 0) {
    const avgEntryDiff =
      bothExchanges.reduce((s, m) => s + (m.entryDiffPct ?? 0), 0) /
      bothExchanges.length;
    const avgConfDelta =
      bothExchanges.reduce((s, m) => s + Math.abs(m.confDelta ?? 0), 0) /
      bothExchanges.length;
    console.log(
      `  Matched signals: avg entry diff=${fmt(avgEntryDiff, 4)}% | avg |conf delta|=${fmt(avgConfDelta, 3)}`,
    );
  }
  ...
  if (total === 0) {
    ...
    console.log(
      "  This is normal — signals are rare. Try --live mode for continuous comparison.",
    );
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `  Mode:       ${isLive ? "LIVE (backfill + WS)" : "SNAPSHOT (backfill only)"}`,
  );
