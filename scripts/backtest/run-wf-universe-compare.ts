...
function resolveCandleCounts(): Record<CandleInterval, number> {
  ...
  if (ext === "1" || ext === "true" || ext === "yes")
    return CANDLE_COUNTS_EXTENDED;
...
function resolveFixedParams(): StrategyParams {
  ...
  try {
    ...
    log.error(
      "wf-compare",
      "WF_COMPARE_STRATEGY_PARAMS must be valid JSON object",
    );
    process.exit(1);
  }
}
...
function printComparisonTable(
  a: WfUniverseRunSummary,
  b: WfUniverseRunSummary,
): void {
  ...
  console.log(
    "  WALK-FORWARD UNIVERSE COMPARISON (fixed params, OOS aggregate)",
  );
  console.log("=".repeat(88));
  console.log(
...
async function main() {
  const baselineCoins = (process.argv[2] ?? WF_COMPARE_BASELINE_10.join(","))
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  const subsetCoins = (process.argv[3] ?? WF_COMPARE_SUBSET_TOP3.join(","))
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  console.log(
    `  History: ${extended ? "extended (WF_COMPARE_EXTENDED_HISTORY)" : "default"}`,
  );
  console.log(
    `  Params: ${JSON.stringify(fixedParams)} (empty object = config.ts defaults)`,
  );
  console.log("=".repeat(60));
  ...
  const criterionB =
    subsetCoins.length === 3 &&
    subsetCoins.every((c) => ["BTC", "ETH", "SOL"].includes(c))
  ...
  const summaryA = runWfOnce(
    candlesA,
    baselineCoins,
    "A_baseline_10",
    "Full 10-coin baseline",
    fixedParams,
  );
  const summaryB = runWfOnce(
    candlesB,
    subsetCoins,
    "B_subset",
    criterionB,
