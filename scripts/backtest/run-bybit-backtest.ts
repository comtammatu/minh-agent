...
async function main() {
  ...
  const totalBatches =
    COINS.length *
    TIMEFRAMES.reduce(
      (s, tf) => s + Math.ceil((CANDLE_COUNTS[tf] ?? 5000) / 1000),
      0,
    );
  ...
  console.log(
    `  Coins: ${COINS.length} (${COINS.slice(0, 5).join(", ")} ... ${COINS.slice(-3).join(", ")})`,
  );
  console.log(`  Timeframes: ${TIMEFRAMES.join(", ")}`);
  console.log(`  Coverage: 5m=30d | 15m=6mo | 1h=7mo | 4h=28mo`);
  console.log(`  Setup engine: ${strategy}`);
  console.log(
    `  Commission: ${(BYBIT_COMMISSION_PCT * 100).toFixed(3)}% (Bybit taker)`,
  );
  console.log(
    `  Estimated API batches: ~${totalBatches} (fetch may take 5-15 min)`,
  );
  console.log("=".repeat(60));
  ...
  if (totalCandles === 0) {
    log.error(
      "bb-backtest",
      "No candle data fetched. Check network / Bybit API availability.",
    );
    process.exit(1);
  }
