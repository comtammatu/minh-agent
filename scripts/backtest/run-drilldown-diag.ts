...
async function fetchAllCandles(
  ...
  for (const coin of coins) {
    for (const tf of allTFs) {
      ...
      log.info(
        "diag",
        `[${done}/${total}] ${coin} ${tf}: ${candles.length} candles`,
      );
    }
  }
  return result;
...
async function main() {
  const coins = (
    process.argv[2] ?? "BTC,ETH,SOL,AVAX,LINK,ARB,APT,BNB,DOT,ATOM"
  ).split(",");
  ...
  if (totalCandles === 0) {
    log.error("diag", "No candle data. Aborting.");
    process.exit(1);
  }
  log.info(
    "diag",
    `Total: ${totalCandles} candles across ${allCandles.size} series`,
  );
  ...
  const tradesByMode = oosTrades.reduce(
    (acc, t) => {
  ...
  console.log(
    `  No break:        ${d.scan4h_no_break} (${pct(d.scan4h_no_break, d.scan4h_calls)})`,
  );
  console.log(
    `  No zones:        ${d.scan4h_no_zones} (${pct(d.scan4h_no_zones, d.scan4h_calls)})`,
  );
  ...
  console.log(
    `  No HTF POIs:        ${d.scan15m_no_htf_pois} (${pct(d.scan15m_no_htf_pois, d.scan15m_calls)})`,
  );
  console.log(`  POIs expired:       ${d.scan15m_pois_expired}`);
  console.log(`  POIs checked:       ${d.scan15m_pois_checked}`);
  console.log(
    `  Not at zone:        ${d.scan15m_not_at_zone} (${pct(d.scan15m_not_at_zone, d.scan15m_pois_checked)})`,
  );
  console.log(
    `  No confirm break:   ${d.scan15m_no_confirm_break} (${pct(d.scan15m_no_confirm_break, d.scan15m_pois_checked)})`,
  );
  ...
  console.log(
    `  No confirmed POIs:  ${d.scan5m_no_confirmed_pois} (${pct(d.scan5m_no_confirmed_pois, d.scan5m_calls)})`,
  );
  ...
  console.log(
    `  ${d.scan4h_pois_registered} → ${d.scan15m_confirmed} → ${d.scan5m_signals}`,
  );
  const convRate15m =
    d.scan4h_pois_registered > 0
      ? ((d.scan15m_confirmed / d.scan4h_pois_registered) * 100).toFixed(2)
      : "N/A";
  const convRate5m =
    d.scan15m_confirmed > 0
  ...
  if (wfCoinRows.length === 0) {
    ...
    console.log(
      `  ${"coin".padEnd(8)} | ${"trades".padStart(6)} | ${"PF".padStart(8)} | ${"WR".padStart(7)} | ${"net PnL".padStart(10)}`,
    );
    ...
    const profitable = wfCoinRows
      .filter((w) => w.netPnl > 0)
      .map((w) => w.coin);
    const losing = wfCoinRows.filter((w) => w.netPnl <= 0).map((w) => w.coin);
    console.log(
      `  Profitable coins: ${profitable.length ? profitable.join(", ") : "(none)"}`,
    );
    console.log(
      `  Losing / flat:    ${losing.length ? losing.join(", ") : "(none)"}`,
    );
  }
  ...
  console.log("\n── COMPARISON ──");
  console.log(
    `  Full run:     ${result.tradeCount} trades (PF ${formatPf(result.oosPF)})`,
  );
  console.log(
    `  Isolated 5m:  ${isoResult.tradeCount} trades (PF ${formatPf(isoResult.oosPF)})`,
  );
  ...
  log.info(
    "diag",
    "Running raw 5m backtest (no walk-forward, all candle data)...",
  );
  ...
  console.log(
    `  PF:          ${rawMetrics.profitFactor === Infinity ? "Inf" : rawMetrics.profitFactor.toFixed(3)}`,
  );
  ...
  log.info(
    "diag",
    "Running raw 1h-only backtest (no walk-forward, all candle data)...",
  );
  ...
  console.log(
    `  Trade count: ${raw1hMetrics.totalTrades} (1h interval rows: ${raw1hTrades.length})`,
  );
  console.log(
    `  PF:          ${raw1hMetrics.profitFactor === Infinity ? "Inf" : raw1hMetrics.profitFactor.toFixed(3)}`,
  );
  ...
  if (rawCoinRows.length === 0) {
    ...
    console.log(
      `  ${"coin".padEnd(8)} | ${"trades".padStart(6)} | ${"PF".padStart(8)} | ${"WR".padStart(7)} | tier`,
    );
  ...
  const mTop3 =
    top3Trades.length > 0
      ? computeMetrics(top3Trades, baseConfig.initialCapital)
      : null;
  const mRest =
    restTrades.length > 0
      ? computeMetrics(restTrades, baseConfig.initialCapital)
      : null;

  console.log("\n── Top-3 (BTC, ETH, SOL) vs remaining coins (raw 1h) ──");
  console.log(
    `  ${"tier".padEnd(12)} | ${"trades".padStart(6)} | ${"PF".padStart(8)} | ${"WR".padStart(7)} | ${"net PnL".padStart(10)}`,
  );
  ...
  if (mTop3) {
    ...
    console.log(
      `  ${"Top-3".padEnd(12)} | ${String(0).padStart(6)} | ${"N/A".padStart(8)} | ${"N/A".padStart(7)} | ${"N/A".padStart(10)}`,
    );
  }
  if (mRest) {
    console.log(
    ...
    console.log(
      `  ${"Rest".padEnd(12)} | ${String(0).padStart(6)} | ${"N/A".padStart(8)} | ${"N/A".padStart(7)} | ${"N/A".padStart(10)}`,
    );
  }
  ...
  if (mTop3 && mRest) {
    if (
      mTop3.profitFactor > mRest.profitFactor &&
      mTop3.netPnl > mRest.netPnl
    ) {
      decisionLine =
      ...
      decisionLine =
        "Mixed tier metrics — rely on per-coin table + WF OOS before changing universe.";
    }
  } else if (mTop3 && !mRest) {
    decisionLine =
      "Only Top-3 coins have raw 1h trades in this dataset — compare WF OOS for coin selection.";
  } else {
  ...
  const top3PfBelow1 = Number.isFinite(top3Pf) && top3Pf < 1;
  ...
  } else if (top3Trades.length > 0 && Number.isFinite(top3Pf) && top3Pf >= 1) {
...
function formatPf(pf: number): string {
  ...
  if (!Number.isFinite(pf)) return "N/A";
  return pf.toFixed(3);
}
...
function printExitReasonBreakdown(trades: BacktestTrade[]): void {
  ...
  for (const [reason, v] of sorted) {
    const sign = v.pnl >= 0 ? "+" : "";
    console.log(
      `    ${reason.padEnd(14)} | n=${String(v.count).padStart(4)} | net ${sign}$${v.pnl.toFixed(2)}`,
    );
  }
}
...
function printConfluenceGradeBreakdown(trades: BacktestTrade[]): void {
  ...
  for (const [grade, v] of sorted) {
    const sign = v.pnl >= 0 ? "+" : "";
    console.log(
      `    ${grade.padEnd(8)} | n=${String(v.count).padStart(4)} | net ${sign}$${v.pnl.toFixed(2)}`,
    );
  }
}
...
function printMaxConsecutiveLossStreak(trades: BacktestTrade[]): void {
  ...
  console.log(
    `  Max consecutive losing trades (pnl <= 0, exit time order): ${maxStreak}`,
  );
}
