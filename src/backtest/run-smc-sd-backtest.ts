...
async function main() {
  ...
  for (const coin of COINS) {
    for (const tf of TIMEFRAMES) {
      ...
      console.log(`OK (${candles.length} candles, ${new Date(candles[0]!.t).toISOString().slice(0,10)} → ${new Date(candles.at(-1)!.t).toISOString().slice(0,10)})`)
    }
  }
  ...
  if (result.trades.length > 0) {
    ...
    console.log('  Coin    TF    Side   Entry         SL%    TP%    R:R    PnL        Exit')
  ...
  // ── Verdict ────────────────────────────────────────────────────────────────
  console.log('\n====================================================')
  const passes = m.expectancy > 0 && m.winRate >= 0.30 && m.maxDrawdown < 0.20
  console.log(`  VERDICT: ${passes ? '✅ PASS' : '❌ FAIL'}`)
  if (!passes) {
    if (m.expectancy <= 0)  console.log('  ⚠️  Negative expectancy')
    if (m.winRate < 0.30)   console.log(`  ⚠️  Win rate ${(m.winRate*100).toFixed(1)}% < 30% floor`)
    if (m.maxDrawdown >= 0.20) console.log(`  ⚠️  Max drawdown ${(m.maxDrawdown*100).toFixed(1)}% ≥ 20% limit`)
  }
  console.log('====================================================\n')
}

main().catch(err => {
  log.error('backtest', `Fatal: ${err instanceof Error ? err.message : String(err)}`)
  process.exit(1)
})
