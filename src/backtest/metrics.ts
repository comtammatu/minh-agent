...
import type { BacktestMetrics, BacktestTrade, EquityPoint } from "./types.js";
...
export function computeMetrics(
  ...
  // Drawdown
  const { maxDrawdown, maxDrawdownDuration } = computeDrawdown(
    trades,
    initialCapital,
  );
...
export function buildEquityCurve(
  ...
  const curve: EquityPoint[] = [
    { ts: trades[0]?.entryTime ?? 0, equity: initialCapital },
  ];
