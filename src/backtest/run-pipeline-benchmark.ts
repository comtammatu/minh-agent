...
async function main(): Promise<void> {
  ...
  const replayEvents = buildReplayEvents(candleMap, options.coins, options.timeframes)
  ...
  console.log(` Trim tail:  ${(options.outlierTrimRatio * 100).toFixed(2)}% per side`)
  ...
  for (const [interval, values] of byIntervalDurations) {
    ...
    byIntervalRobust[interval] = summarizeLatency(trimOutliers(values, options.outlierTrimRatio))
  }
  ...
  if (shouldPersist) {
    const outPath = options.outputPath !== null
      ...
      : join(process.cwd(), 'results', `pipeline-benchmark-${new Date().toISOString().replace(/[:.]/g, '-')}.json`)
...
function runReplay(
  ...
  onCandleTick: (coin: string, interval: CandleInterval, candle: ReplayEvent['candle']) => void,
