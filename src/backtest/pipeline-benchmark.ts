...
import { TIMEFRAME_MS, SIGNAL_TIMEFRAMES } from '../config.js'

export const VALID_INTERVALS: readonly CandleInterval[] = ['1m', '5m', '15m', '1h', '4h', '1d']
...
export function parseBenchmarkArgs(argv: readonly string[]): BenchmarkOptions {
  ...
  return {
    ...
    measuredRuns: Math.max(1, parsePositiveInt(argv, '--runs', DEFAULT_MEASURED_RUNS)),
    outlierTrimRatio: parseTrimRatio(argv, '--trim', DEFAULT_OUTLIER_TRIM_RATIO),
    outputPath: parseStringArg(argv, '--out') ?? parseStringArg(argv, '--output'),
  ...
  for (let coinIdx = 0; coinIdx < coins.length; coinIdx++) {
    ...
    for (let tfIdx = 0; tfIdx < timeframes.length; tfIdx++) {
      ...
      for (let i = 0; i < barsPerSeries; i++) {
        ...
        const volumeCycle = 1 + Math.abs(Math.sin(i / 13)) * 0.8 + Math.abs(Math.cos(i / 31)) * 0.4
...
export function buildReplayEvents(
  ...
  const intervalOrder = new Map<CandleInterval, number>(VALID_INTERVALS.map((tf, idx) => [tf, idx]))

  events.sort((a, b) => {
    const tDiff = a.candle.t - b.candle.t
    if (tDiff !== 0) return tDiff

    const tfDiff = (intervalOrder.get(a.interval) ?? 0) - (intervalOrder.get(b.interval) ?? 0)
 ...
 */
export function trimOutliers(samplesMs: readonly number[], trimRatio: number): number[] {
...
function parsePositiveInt(argv: readonly string[], flag: string, fallback: number): number {
...
function parseNonNegativeInt(argv: readonly string[], flag: string, fallback: number): number {
  const n = parseIntArg(argv, flag)
  if (n === null) return fallback
  if (!Number.isFinite(n) || n < 0) return fallback
  return Math.floor(n)
...
function parseIntWithDefault(argv: readonly string[], flag: string, fallback: number): number {
  const n = parseIntArg(argv, flag)
  if (n === null) return fallback
...
function parseTrimRatio(argv: readonly string[], flag: string, fallback: number): number {
  const n = parseNumberArg(argv, flag)
  if (n === null) return fallback
  if (!Number.isFinite(n) || n < 0) return fallback
  // Keep in [0, 0.20] so robust view is informative and never over-trims.
  return Math.min(n, 0.20)
}
