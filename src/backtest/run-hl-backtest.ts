...
import type { BacktestConfig, StrategyType, WalkForwardConfig } from './types.js'
  ...
  for (const coin of coins) {
    for (const tf of allTFs) {
      ...
      if (candles === null || candles.length === 0) {
        log.warn('hl-backtest', `[${done}/${total}] ${coin} ${tf}: FAILED — skipping`)
        continue
      }
      ...
      log.info('hl-backtest', `[${done}/${total}] ${coin} ${tf}: ${candles.length} candles (${dateRange})`)
    }
  }
...
function printTradeDetails(trades: import('./types.js').BacktestTrade[]): void {
  ...
  for (const [reason, { count, pnl }] of [...byExit.entries()].sort((a, b) => b[1].count - a[1].count)) {
    console.log(`    ${reason.padEnd(15)} ${String(count).padStart(3)} trades  PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`)
  }

  // Breakdown by coin
  const byCoin = new Map<string, { count: number; pnl: number; wins: number }>()
  for (const t of trades) {
    const c = byCoin.get(t.coin) ?? { count: 0, pnl: 0, wins: 0 }
    c.count++; c.pnl += t.pnl; if (t.pnl > 0) c.wins++
    byCoin.set(t.coin, c)
  }
  console.log('\n  Per-coin breakdown:')
  for (const [coin, { count, pnl, wins }] of [...byCoin.entries()].sort((a, b) => b[1].pnl - a[1].pnl)) {
    const wr = count > 0 ? (wins / count * 100).toFixed(0) : '0'
    console.log(`    ${coin.padEnd(7)} ${String(count).padStart(3)} trades  WR: ${wr.padStart(3)}%  PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`)
  }
  ...
  for (const t of trades) {
    ...
    tf.count++; tf.pnl += t.pnl; if (t.pnl > 0) tf.wins++
    byTF.set(t.interval, tf)
  }
  console.log('\n  Per-TF breakdown:')
  for (const [tf, { count, pnl, wins }] of [...byTF.entries()].sort((a, b) => b[1].pnl - a[1].pnl)) {
    const wr = count > 0 ? (wins / count * 100).toFixed(0) : '0'
    console.log(`    ${tf.padEnd(5)} ${String(count).padStart(3)} trades  WR: ${wr.padStart(3)}%  PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`)
  }

  // Breakdown by side
  const bySide = new Map<string, { count: number; pnl: number; wins: number }>()
  for (const t of trades) {
  ...
  for (const [side, { count, pnl, wins }] of bySide) {
    const wr = count > 0 ? (wins / count * 100).toFixed(0) : '0'
    console.log(`    ${side.padEnd(6)} ${String(count).padStart(3)} trades  WR: ${wr.padStart(3)}%  PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)}`)
  }
...
function runStrategyBacktest(
  ...
  log.info('hl-backtest', `${strategy}: ${diagResult.trades.length} trades on full dataset`)
...
async function main() {
  ...
  if (extraHTFs.length > 0) {
    log.info('hl-backtest', `Extra HTF intervals for bias warmup: ${extraHTFs.join(', ')}`)
  }
  ...
  if (totalCandles === 0) {
    log.error('hl-backtest', 'No candle data fetched. Check network / HL API availability.')
    process.exit(1)
  }
...
main().catch(err => {
  log.error('hl-backtest', `Fatal: ${err instanceof Error ? err.message : String(err)}`)
  process.exit(1)
})
