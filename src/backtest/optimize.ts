...
export function runTrial(
  ...
  try {
    const result: WalkForwardResult = walkForward(
      trainCandles,
      wfConfigWithParams,
    );
...
export function scoreOosCandidate(trial: TrialResult): number {
  ...
  const tradeFactor = clamp01(
    trial.tradeCount / OPTIMIZER_SELECTION_OOS_TRADE_TARGET,
  );
...
export function selectTopN(trials: TrialResult[], n: number): TrialResult[] {
  return trials
    .filter((t) => !t.error && t.tradeCount >= 5 && Number.isFinite(t.oosPF))
    .sort((a, b) => {
...
export function scoreHoldoutRobustness(trial: HoldoutResult): number {
  ...
  const holdoutTradeFactor = clamp01(
    trial.holdoutTrades / OPTIMIZER_HOLDOUT_TRADE_TARGET,
  );
  const holdoutDdPenalty = clamp01(1 - trial.holdoutMaxDD);
  const overfitPenalty = clamp01(
    holdoutPf / Math.max(normalizePf(trial.oosPF), 0.01),
  );
...
export function isRobustHoldout(trial: HoldoutResult): boolean {
  return (
    trial.holdoutPF >= OPTIMIZER_HOLDOUT_MIN_PF &&
...
export function rankHoldoutResults(trials: HoldoutResult[]): HoldoutResult[] {
  return [...trials].sort((a, b) => {
    ...
    if (scoreDiff !== 0) return scoreDiff;
    if (b.holdoutTrades !== a.holdoutTrades)
      return b.holdoutTrades - a.holdoutTrades;
...
export function validateHoldout(
  ...
  for (const trial of topTrials) {
    ...
    try {
      ...
      const holdoutOosTrades = result.windows.flatMap(
        (w) => w.testTrades ?? [],
      );
      const holdoutTradesByMode = holdoutOosTrades.reduce(
        (acc, t) => {
          const mode = inferScanMode(t.interval);
          acc[mode] = (acc[mode] ?? 0) + 1;
          return acc;
      ...
      log.warn(
        "optimizer",
        `Holdout validation failed for trial ${trial.trialIndex}: ${err instanceof Error ? err.message : String(err)}`,
      );
      results.push({
        ...trial,
        holdoutPF: 0,
...
export function formatConsoleTable(topN: HoldoutResult[]): string {
  ...
  for (let i = 0; i < topN.length; i++) {
    ...
    const modeStr = t.holdoutTradesByMode
      ? Object.entries(t.holdoutTradesByMode)
          .map(([m, n]) => `${m}:${n}`)
          .join(" ")
      : "";
...
async function writeTrialsToDB(
  ...
  try {
    ...
    log.info(
      "optimizer",
      `Wrote ${trials.length} trials to PostgreSQL (run ${runId})`,
    );
  } catch (err) {
    log.warn(
      "optimizer",
      `DB write failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
async function main() {
  ...
  const candidateCount = Math.max(
    ...
    Math.min(
      ...
      Math.max(
        OPTIMIZER_CANDIDATE_MIN,
        Math.ceil(numTrials * OPTIMIZER_CANDIDATE_FRACTION),
      ),
    ),
  );
  ...
  if (allCandles.size < expectedSeries) {
    log.warn(
      "optimizer",
      `Only ${allCandles.size}/${expectedSeries} series loaded — some coins may have failed`,
    );
    const loadedCoins = new Set(
      [...allCandles.keys()].map((k) => k.split("|")[0] ?? ""),
    );
    const missingCoins = coins.filter((c) => !loadedCoins.has(c));
    if (missingCoins.length > 0)
      log.warn("optimizer", `Missing coins: ${missingCoins.join(", ")}`);
  }
  log.info(
    "optimizer",
  ...
  log.info(
    "optimizer",
    `Train series: ${trainCandles.size}, Holdout series: ${holdoutCandles.size}`,
  );
  ...
  for (let i = 0; i < paramSets.length; i++) {
    ...
    if (result.error) {
      ...
      log.info(
        "optimizer",
        `  PF=${result.oosPF.toFixed(2)} DD=${(result.maxDD * 100).toFixed(1)}% trades=${result.tradeCount} (${result.durationMs}ms)`,
      );
    }
  }
  ...
  log.info(
    "optimizer",
    `${holdoutCandidates.length} candidate trials selected for holdout validation...`,
  );

  const holdoutValidated = validateHoldout(
    holdoutCandles,
    baseConfig,
    wfConfig,
    holdoutCandidates,
  ...
  if (plateauDetected) {
    log.warn(
      "optimizer",
      "PLATEAU DETECTED: PF variance < 0.05 in top-20. Consider Optuna/TPE for smarter sampling.",
    );
  }
  ...
  console.log(
    `Successful trials: ${trials.filter((t) => !t.error).length}/${numTrials}`,
  );
  ...
  if (plateauDetected) {
    console.log(
      "⚠ PLATEAU DETECTED — random search exhausted, consider Bayesian optimization",
    );
  }
