 ...
 */
export type { AdvisorSuggestion, AdvisorContext } from './types.js';

// Placeholder client factory (no dep yet to avoid secret requirement).
export function createAdvisorClient() {
  return {
    async suggest(context: import('./types.js').AdvisorContext): Promise<import('./types.js').AdvisorSuggestion[]> {
