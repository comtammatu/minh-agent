...
import type { AgentState, AgentAction, AgentSnapshot } from '../agent/types.js'
...
export function formatGrade(grade: ConfluenceGrade): string {
  switch (grade) {
    case 'A+': return c(ANSI.bold + ANSI.bgMagenta + ANSI.white, ' A+ ')
    case 'A':  return c(ANSI.bold + ANSI.bgGreen + ANSI.white, ' A ')
    case 'B':  return c(ANSI.bold + ANSI.bgBlue + ANSI.white, ' B ')
    case 'C':  return c(ANSI.dim, '[C]')
  }
}
...
export function formatState(state: AgentState): string {
  switch (state) {
    case 'IDLE':        return c(ANSI.dim, '[IDLE]')
    case 'WATCHING':    return c(ANSI.cyan, '[WATCHING]')
    case 'ENTERING':    return c(ANSI.yellow, '[ENTERING]')
    case 'IN_POSITION': return c(ANSI.bold + ANSI.green, '[IN_POSITION]')
    case 'EXITING':     return c(ANSI.yellow, '[EXITING]')
    case 'PAUSED':      return c(ANSI.bold + ANSI.red, '[PAUSED]')
  }
}
...
export function formatAction(action: AgentAction): string | null {
  ...
  switch (eventType) {
    ...
    case 'enter': {
      ...
      const price = details.fillPrice != null ? Number(details.fillPrice).toFixed(4) : '?'
...
export function formatAgentStatus(snapshot: AgentSnapshot): string {
  ...
  const pauseTag = globalPaused
    ? c(ANSI.bold + ANSI.red, ' PAUSED')
    : ''

  // Active coins summary
  const activeParts = active.map(([coin, v]) =>
    `${coin}${formatState(v.state)}`,
  )
