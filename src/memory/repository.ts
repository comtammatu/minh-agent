...
export async function queryMemories(
  ...
  if (query.text)
    conditions.push(
      sql`search_vec @@ plainto_tsquery('english', ${query.text})`,
    );
  ...
  if (rows.length > 0) {
    ...
    sql`
      ...
      WHERE id = ANY(${ids})
    `.catch((err) =>
      log.error("memory", `Failed to bump access: ${(err as Error).message}`),
    );
  }
...
export async function pruneMemories(
  ...
  if (count > 0) {
    log.info(
      "memory",
      `Pruned ${count} stale memories (>${retentionDays}d, low importance)`,
    );
  }
  return count;
}
