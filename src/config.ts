...
function parseHeartbeatEnv(name: string, fallback: number): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    throw new Error(
      `${name} must be a positive number of milliseconds (received "${raw}")`,
    );
  }
  return n;
}
...
export const SMC_ICT_KILLZONES: ReadonlyArray<{
  ...
  { name: "us-overlap", startUTC: 13, endUTC: 16, bonus: 0.1 }, // London/US overlap: highest vol
...
export const PIPELINE_BENCH_CI_BASELINE_PATH =
  "results/baselines/pipeline-benchmark-baseline.json";
...
export const PIPELINE_BENCH_CI_TIMEFRAMES = [
  "5m",
  "15m",
  "1h",
  "4h",
...
export const PIPELINE_BENCH_CI_METRIC_MODE: PipelineBenchmarkMetricMode =
  "robust";
...
export const MAX_STOP_DISTANCE_PCT = 0.1;
...
export const MULTI_TP_SPLIT = [0.35, 0.25, 0.4] as const;
...
export function getActiveExchange(): ExchangeId {
  ...
  if (!raw)
    throw new Error("ACTIVE_EXCHANGE env is required. Set to HL or BB.");
  if (raw !== "HL" && raw !== "BB")
    throw new Error(`Unknown ACTIVE_EXCHANGE: "${raw}". Valid values: HL, BB`);
  return raw;
}
...
export const OPTIMIZER_CANDIDATE_FRACTION = 0.2;
...
export function validateWindowPolicies(): void {
  ...
  for (const tf of TIMEFRAMES) {
    ...
    if (hot < plan + 2) {
      errors.push(
        `HOT_CACHE_CAP_BARS['${tf}']=${hot} < PLANNING_WINDOW_BARS['${tf}']=${plan} + 2`,
      );
    }
  ...
  if (errors.length > 0) {
    throw new Error(
      `Window policy invariant violation:\n  ${errors.join("\n  ")}`,
    );
  }
}
