...
export class HealthMonitor {
  ...
  recordError(component: ComponentName, error: string): void {
    ...
    const staleMs =
      component === "exchange" ? HEALTH.exchangeStaleMs : HEALTH.dbStaleMs;
  ...
  getReport(): HealthReport {
    ...
    for (const [name, health] of Object.entries(this.components)) {
      const staleMs =
        name === "exchange" ? HEALTH.exchangeStaleMs : HEALTH.dbStaleMs;
      health.status = computeComponentStatus(health, staleMs, now);
    }

    const memoryStatus = computeMemoryStatus(rssBytes);
    const componentStatuses = Object.values(this.components).map(
      (h) => h.status,
    );
  ...
  startPeriodicCheck(): void {
    ...
    this.checkTimer = setInterval(() => {
      ...
      if (report.overall === "critical") {
        log.error(
          "health",
          `CRITICAL — overall=${report.overall} rss=${formatBytes(report.rssBytes)}`,
        );
        for (const [name, h] of Object.entries(report.components)) {
          if (h.status === "critical") {
            log.error(
              "health",
              `  ${name}: ${h.status} errors=${h.consecutiveErrors} lastErr=${h.lastError ?? "none"}`,
            );
          }
        }
        ...
        for (const [name, h] of Object.entries(report.components)) {
          if (h.status !== "ok") {
            log.warn(
              "health",
              `  ${name}: ${h.status} errors=${h.consecutiveErrors}`,
            );
          }
        }
      }
      ...
      if (memStatus === "critical") {
        log.error(
          "health",
          `RSS CRITICAL: ${formatBytes(report.rssBytes)} — consider restart`,
        );
