...
import type { CoinContext, GlobalContext } from "./types.js";
...
describe("handleWatching", () => {
  it("goes to IDLE on setup_invalidated", () => {
    ...
    const result = handleWatching(
      ctx,
      { type: "setup_invalidated", setupId: setup.id, reason: "zone-broken" },
      makeGlobal(),
    );
    expect(result.nextState).toBe("IDLE");
  });
  ...
  it("stays WATCHING on low-grade replacement setup", () => {
    ...
    const result = handleWatching(
      ctx,
      { type: "setup_detected", setup: weaker },
      makeGlobal(),
    );
  ...
  it("stays WATCHING with skip when new setup is not higher confidence", () => {
    ...
    const result = handleWatching(
      ctx,
      { type: "setup_detected", setup: sameConf },
      makeGlobal(),
    );
    expect(result.nextState).toBe("WATCHING");
    const skip = result.actions.find(
  ...
  it("upgrades setup on higher confidence", () => {
    ...
    const result = handleWatching(
      ctx,
      { type: "setup_detected", setup: better },
      makeGlobal(),
    );
    expect(result.nextState).toBe("WATCHING");
...
describe("handleEntering", () => {
  it("transitions to IN_POSITION on order_filled", () => {
    const ctx = makeCoinCtx({
      ...
      activeSetup: makeSetup(),
    });
    const result = handleEntering(
      ctx,
      {
        type: "order_filled",
        orderId: "ord-1",
    ...
    expect(
      result.actions.some(
        (a) => a.type === "log_journal" && a.eventType === "enter",
      ),
    ).toBe(true);
  });
  ...
  it("transitions to IDLE + cancels on order_timeout", () => {
    ...
    const result = handleEntering(
      ctx,
      { type: "order_timeout", orderId: "ord-1" },
      makeGlobal(),
    );
    expect(result.nextState).toBe("IDLE");
  ...
  it("cancels order on tick timeout", () => {
    ...
    expect(
      result.actions.some(
        (a) => a.type === "cancel_order" && a.orderId === "ord-1",
      ),
    ).toBe(true);
  });
...
describe("handleInPosition", () => {
  ...
  it("closes position on setup_invalidated", () => {
    ...
    expect(
      result.actions.some(
        (a) => a.type === "close_position" && a.positionId === "pos-1",
      ),
    ).toBe(true);
  });
...
describe("handleExiting", () => {
  ...
  it("retries close on exit timeout (stays EXITING)", () => {
    ...
    expect(
      result.actions.some(
        (a) => a.type === "log_journal" && a.eventType === "error",
      ),
    ).toBe(true);
  });
});
...
describe("TradingAgent", () => {
  ...
  it("pauseAll pauses all coins", () => {
    agent.dispatch("BTC", {
      type: "setup_detected",
      setup: makeSetup({ confluenceGrade: "A" }),
    });
    agent.dispatch("ETH", {
      type: "setup_detected",
      setup: makeSetup({
  ...
  it("crash recovery: DB position not on exchange → IDLE", () => {
    ...
    expect(
      actions.some(
        (a: any) =>
          a.eventType === "exit" &&
          a.details?.reason === "crash_recovery_closed",
      ),
    ).toBe(true);
  });
  ...
  it("checkCircuitBreakers does NOT pause IN_POSITION coins (R5)", () => {
    ...
    const btcCtx = (agent as unknown as { coins: Map<string, CoinContext> })
      .coins;
    btcCtx.set(
      "BTC",
      makeCoinCtx({ state: "IN_POSITION", positionId: "pos-1", coin: "BTC" }),
    );
  ...
  it("position close events update dailyPnl via recordPnl", () => {
    ...
    agent.dispatch("ETH", {
      type: "setup_detected",
      setup: makeSetup({ coin: "ETH", confluenceGrade: "A" }),
    });
    agent.dispatch("ETH", {
      type: "order_filled",
      positionId: "pos-2",
  ...
  it("checkCircuitBreakers emits journal action", () => {
    ...
    const cbActions = actions.filter(
      (a: any) => a.eventType === "circuit_break",
    );
...
describe("TradingAgent — correlation guard", () => {
  ...
  it("blocks entry when correlated positions exceed limit", () => {
    ...
    agent.recoverFromCrash(
      [
        ...
        { coin: "STX", size: 100, entryPrice: 1.5 },
      ],
      [
        { coin: "BTC", positionId: "p1", side: "long" },
        { coin: "STX", positionId: "p2", side: "long" },
      ],
    );

    // Try to enter ORDI (also btc-ecosystem) — should be blocked
    agent.onSetup(
      makeSetup({ coin: "ORDI", id: "ORDI|1h|ob|long", confluenceGrade: "A" }),
    );
    ...
    const skipActions = actions.filter(
      (a: any) => a.eventType === "skip" && a.coin === "ORDI",
    );
  ...
  it("allows entry for coins in different groups", () => {
    ...
    agent.onSetup(
      makeSetup({ coin: "ETH", id: "ETH|1h|ob|long", confluenceGrade: "A" }),
    );
    expect(agent.getCoinState("ETH")).toBe("ENTERING");
  });
  ...
  it("allows entry for unknown coins (not in any group)", () => {
    ...
    agent.onSetup(
      makeSetup({ coin: "HYPE", id: "HYPE|1h|ob|long", confluenceGrade: "A" }),
    );
