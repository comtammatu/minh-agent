...
export const CHART_RESOLUTION_TO_INTERVAL: Record<
  ChartResolution,
  CandleInterval
> = {
  "1": "1m",
...
export const CHART_INTERVAL_TO_RESOLUTION: Record<
  CandleInterval,
  ChartResolution
> = {
  "1m": "1",
...
export const CHART_SUPPORTED_RESOLUTIONS = Object.keys(
  CHART_RESOLUTION_TO_INTERVAL,
) as ChartResolution[];
...
export function parseTicker(
...
): { exchange: ExchangeId; coin: string } {
  ...
  if (rawExchange !== activeExchange) {
    throw new Error(
      `Ticker exchange ${rawExchange} does not match active runtime exchange ${activeExchange}`,
    );
  }
...
export function buildChartSymbolInfo(
  exchange: ExchangeId,
  coin: string,
): ChartSymbolInfo {
  return {
...
function buildMarkText(entry: JournalEntry): string {
  const side =
    typeof entry.details.side === "string" ? entry.details.side : null;
  const interval =
    typeof entry.details.interval === "string" ? entry.details.interval : null;
  ...
  const suffix = [side, interval]
    .filter((value): value is string => value !== null)
    .join(" ");
  return suffix.length > 0
    ? `${entry.eventType.toUpperCase()} ${suffix}`
...
function lineText(prefix: string, price: number, suffix?: string): string {
  return suffix
    ? `${prefix} ${price.toFixed(4)} · ${suffix}`
    : `${prefix} ${price.toFixed(4)}`;
}
