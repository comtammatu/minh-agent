    ...
    async order(params: unknown) {
      ...
      if (!orderResponse) {
        return {
          response: {
            data: {
              statuses: [{ filled: { totalSz: '0.1', avgPx: '50000', oid: 12345 } }],
            },
          },
        }
      }
...
describe('ExchangeService', () => {
  ...
  describe('placeOrder', () => {
    ...
    it('should pass correct params to SDK', async () => {
      ...
      const call = lastOrderCall as { orders: { a: number; b: boolean; r: boolean; t: { limit: { tif: string } } }[]; grouping: string }
    ...
    it('should use Gtc for limit orders', async () => {
      ...
      const call = lastOrderCall as { orders: { a: number; b: boolean; t: { limit: { tif: string } } }[] }
  ...
  describe('placeTrigger', () => {
    it('should place SL trigger (market) with normalTpsl grouping', async () => {
      ...
      const call = lastOrderCall as { orders: { r: boolean; t: { trigger: { isMarket: boolean; tpsl: string } } }[]; grouping: string }
    ...
    it('should place TP trigger (limit) with normalTpsl grouping', async () => {
      ...
      const call = lastOrderCall as { orders: { t: { trigger: { isMarket: boolean; tpsl: string } } }[]; grouping: string }
  ...
  describe('cancel', () => {
    ...
    it('should cancel by cloid', async () => {
      const result = await svc.cancelByCloid('BTC', '0x1234567890abcdef1234567890abcdef')
      expect(result.success).toBe(true)

      const call = lastCancelByCloidCall as { cancels: { asset: number; cloid: string }[] }
  ...
  describe('modifyTrigger', () => {
    it('should modify SL trigger with correct params', async () => {
      const result = await svc.modifyTrigger('BTC', 12345, 'short', 49500, 0.1, true, 'sl')

      expect(result.success).toBe(true)
      expect(result.status).toBe('modified')

      const call = lastModifyCall as { oid: number; order: { a: number; b: boolean; r: boolean; t: { trigger: { isMarket: boolean; tpsl: string } } } }
  ...
  describe('getPositions', () => {
    ...
    it('should filter out closed positions (size = 0)', async () => {
      clearinghouseResponse = {
        marginSummary: { accountValue: '10000', totalNtlPos: '0', totalMarginUsed: '0', totalRawUsd: '10000' },
        withdrawable: '10000',
        assetPositions: [
          {
            type: 'oneWay',
