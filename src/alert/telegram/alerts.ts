...
export function checkTelegramConfig(): boolean {
  ...
  if (!config) {
    log.warn(
      "telegram",
      `Alerts disabled — set ${TELEGRAM.tokenEnv} and ${TELEGRAM.chatIdEnv} env vars`,
    );
    return false;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 400 && resBody.includes("can't parse entities")) {
        log.warn(
          "telegram",
          `${parseMode} parse error — retrying as plain text`,
        );
        let plain = message;
        ...
        log.error(
          "telegram",
          `Plain text retry also failed: HTTP ${retryRes.status}`,
        );
        return false;
      }
...
function formatSignalPriceHtml(
  value: unknown,
  esc: (s: string) => string,
): string {
...
function formatSignalRiskReward(
...
export function formatAlert(
  ...
  switch (eventType) {
    case "signal": {
      ...
      const sideLine =
        sideRaw !== "" ? `<b>${h(sideRaw.toUpperCase())}</b>` : "—";
      const patternLine =
      ...
      const replacedLine =
        details.replaced != null
          ? `Replaces: <code>${h(String(details.replaced))}</code>`
          : null;
      const text = [
        `🔍 <b>SETUP DETECTED</b>`,
        ``,
    ...
    case "circuit_break": {
      ...
      const dailyPnl =
        details.dailyPnl != null ? Number(details.dailyPnl).toFixed(2) : "?";
      const text = [
...
export function formatScheduledBriefingHtml(
  ...
  if (context.attention != null) {
    lines.push(
      ``,
      `⚠️ <b>${h(context.attention.level)}</b>: ${h(context.attention.summary)}`,
    );
  }
  ...
  if (context.liveBuckets.length > 0) {
    ...
    for (const bucket of context.liveBuckets) {
      lines.push(
        `  ${h(bucket.label)}: ${h(String(bucket.count))} (${h(bucket.coin)})`,
      );
    }
  }
  ...
  if (context.operatorRecent.totalActions > 0) {
    ...
    for (const item of context.operatorRecent.items) {
      lines.push(
        `  ${h(item.action)} → ${h(item.target)} (${h(item.source)} ${h(item.at)})`,
      );
    }
  }
