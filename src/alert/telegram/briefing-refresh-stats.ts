...
function deriveState(): BriefingHealthState {
  if (
    failureStreak >= CRITICAL_THRESHOLD ||
    coalescedStreak >= CRITICAL_THRESHOLD
  )
    return "critical";
  if (
    failureStreak >= DEGRADED_THRESHOLD ||
    coalescedStreak >= DEGRADED_THRESHOLD
  )
    return "degraded";
  return "healthy";
}
...
export function getBriefingRefreshIncidents(
  limit = 10,
): BriefingRefreshIncident[] {
...
export function recordBriefingRefreshOutcome(
  ...
  if (next !== "healthy" && prev === "healthy") {
    ...
    const active = [...incidents]
      .reverse()
      .find((i: BriefingRefreshIncident) => i.status === "active");
    if (active != null) {
