...
function helpHandler(): string {
  ...
  const lines = [`*Minh \\(明\\) — Trading Bot*`, ``, `Available commands:`];
...
function formatBriefingRefreshStatsLine(): string {
  ...
  const lastOutcome =
    stats.lastOutcome != null ? stats.lastOutcome.replace(/_/g, " ") : "none";
...
function formatGuardianActivitySummary(
...
function formatBriefingHealthSummary(): string {
  ...
  parts.push(
    `${esc(String(stats.edited))}/${esc(String(stats.requested))} edited`,
  );
...
function formatBriefingDrilldownLine(): string | null {
  ...
  if (health.state === "healthy" && health.recoveredFrom != null) {
    ...
    if (health.recoveredTarget != null)
      pieces.push(`target ${esc(health.recoveredTarget)}`);
    if (health.recoveredAttention != null)
      pieces.push(`attention ${esc(health.recoveredAttention)}`);
  } else {
    if (health.lastKind != null) pieces.push(esc(health.lastKind));
    if (health.lastOutcome != null)
      pieces.push(esc(health.lastOutcome.replace(/_/g, " ")));
    if (health.lastTarget != null)
      pieces.push(`target ${esc(health.lastTarget)}`);
    if (health.lastAttention != null)
      pieces.push(`attention ${esc(health.lastAttention)}`);
  }
...
function statusHandler(): string {
  ...
  try {
    ...
    const watching = coinStates.filter(
      ([, c]) => c.state === "WATCHING",
    ).length;
    const inPos = coinStates.filter(
      ([, c]) => c.state === "IN_POSITION",
    ).length;
    ...
    return [
      ...
      ...(formatBriefingIncidentLine(2) != null
        ? [`Briefing incident: ${formatBriefingIncidentLine(2)}`]
        : []),
      ...(formatBriefingHistoryLine(2) != null
        ? [`Briefing history: ${formatBriefingHistoryLine(2)}`]
        : []),
    ].join("\n");
  } catch {
...
function positionsHandler(): string {
  ...
  try {
    ...
    const lines: string[] = [
      `*Open Positions \\(${esc(String(posMap.size))}\\)*`,
      ``,
    ];
 ...
 */
export function parsePauseCoinArgs(
  args: string,
): { coin: string; durationMs: number; label: string } | null {
...
function pauseHandler(args: string): string {
  ...
  try {
    ...
    if (coinPause) {
      ...
      agent.dispatch(coin, {
        type: "pause",
        reason: `manual via Telegram (${label})`,
      });
      // Schedule auto-resume
      setTimeout(() => {
        try {
          agent.dispatch(coin, { type: "resume" });
        } catch {
          /* agent may be gone */
        }
      }, durationMs);
...
function riskHandler(): string {
  ...
  try {
    ...
    const cbTripped =
      snap.global.totalConsecutiveLosses >= 3 ? "TRIPPED" : "OK";
    ...
    for (const [coin, ctx] of Object.entries(snap.coins)) {
      if (ctx.consecutiveLosses > 0) {
        coinLosses.push(
          `  ${esc(coin)}: ${esc(String(ctx.consecutiveLosses))} consecutive`,
        );
      }
    }
...
function formatGuardianSnapshot(trace: DecisionTrace): string[] {
  ...
  if (positionId != null) {
    ...
    if (pos != null) {
      lines.push(
        `Live: ${esc(pos.coin)} ${esc(pos.side.toUpperCase())} \\| tracked`,
      );
      lines.push(
  ...
  const latestLifecycle = [...trace.timeline]
    .reverse()
    .find((item) => item.actor === "guardian" || item.actor === "executor");
  if (latestLifecycle != null) {
    lines.push(
      `Last lifecycle: ${esc(latestLifecycle.actor)} \\| ${esc(latestLifecycle.summary)}`,
    );
  }
...
async function findLatestOperatorAuditForTrace(
  trace: DecisionTrace,
): Promise<TraceOperatorAudit | null> {
  ...
  const filtered = entries.filter((entry) => {
    ...
    const action =
      typeof entry.details.action === "string" ? entry.details.action : null;
    const target =
      typeof entry.details.target === "string" ? entry.details.target : null;
    ...
    const entryPositionId =
      typeof entry.details.positionId === "string"
        ? entry.details.positionId
        : null;

    if (
      trace.outcome.positionId != null &&
      entryPositionId === trace.outcome.positionId
    )
      return true;
    return false;
  });
  ...
  return {
    ...
    source:
      typeof latest.details.operatorSource === "string"
        ? latest.details.operatorSource
        : null,
    ts: latest.ts,
  };
}

function formatOperatorIntervention(
  audit: TraceOperatorAudit | null,
): string[] {
  const esc = escapeMarkdownV2;
...
function matchesBriefingHealthTarget(
  ...
  if (
    ...
    focus.positionId != null &&
    target.positionId === focus.positionId
  ) {
    return true;
  }
...
function prependBriefingFocusIntro(
  ...
  if (target == null || (target.coin == null && target.positionId == null))
    return reply;
  ...
  return {
    inline_keyboard: [
      [
        {
          text: "➖ Reduce 25%",
          callback_data: `c:trace_reduce25:${positionId}${briefingSuffix}`,
        },
        {
          text: "➗ Reduce 50%",
          callback_data: `c:trace_reduce50:${positionId}${briefingSuffix}`,
        },
      ],
      [
        {
          text: "🛑 Close",
          callback_data: `c:trace_close:${positionId}${briefingSuffix}`,
        },
        {
          text: "🔄 Refresh",
          callback_data: `c:trace_refresh:${positionId}${briefingSuffix}`,
        },
      ],
    ],
  };
...
async function traceHandler(args: string): Promise<string> {
  ...
  if (trace == null) {
    ...
    return focusTarget == null
      ? message
      : prependBriefingFocusIntro("trace", message, focusTarget);
  }
  ...
  return focusedTraceContext == null
    ? reply
    : prependBriefingFocusIntro("trace", reply, focusedTraceContext);
}
...
function getPendingPositionAction(
  chatId: number,
): PendingPositionOperatorAction | null {
...
function buildPendingPositionActionKeyboard(
  action: PendingPositionOperatorAction,
): {
  inline_keyboard: { text: string; callback_data: string }[][];
} {
  const confirmText =
...
function formatPendingPositionActionLabel(
  action: PendingPositionOperatorAction,
): string {
...
function schedulePendingPositionActionExpiry(
  chatId: number,
  timeoutSec: number,
): void {
  setTimeout(
    () => {
      ...
      if (
        ...
        Date.now() - pendingPositionAction.requestedAt >= timeoutSec * 1000
      ) {
        clearPendingPositionAction(chatId);
      }
    },
...
export function requestRemoteOperatorActionWithContext(
  ...
  if ((kind !== "close" && kind !== "reduce") || !positionId)
...
async function operatorHandler(args: string, chatId: number): Promise<string> {
  ...
  try {
    ...
    if (entries.length === 0) {
      ...
      return focusTarget == null
        ? noEntries
        : prependBriefingFocusIntro("operator", noEntries, focusTarget);
    }
    ...
    for (const entry of entries) {
      const action =
        typeof entry.details.action === "string"
          ? entry.details.action
          : entry.eventType;
      const target =
        typeof entry.details.target === "string"
      ...
      const status =
        typeof entry.details.status === "string"
          ...
          : "UNKNOWN";
      const reason =
        typeof entry.details.reason === "string" ? entry.details.reason : null;
      const position =
        typeof entry.details.positionId === "string"
          ? entry.details.positionId
          : null;
      ...
      lines.push(
        `${esc(stamp)} \\| *${esc(status)}* \\| ${esc(action)} ${esc(target)}`,
      );
    ...
    return focusTarget == null
      ? reply
      : prependBriefingFocusIntro("operator", reply, focusTarget);
  } catch {
...
export function getPendingCloseAll(chatId?: number): CloseAllState | null {
  if (chatId == null)
    return pendingCloseAllByChat.values().next().value ?? null;
...
function closeallHandler(_args: string, chatId: number): string {
  ...
  setTimeout(
    () => {
      ...
      if (
        pendingCloseAll &&
        Date.now() - pendingCloseAll.requestedAt >= timeoutSec * 1000
      ) {
        pendingCloseAllByChat.delete(chatId);
      }
    },
  ...
  if (pendingPositionAction) {
    ...
    if (pos == null) {
      await logOperatorAuditEntry(
        actionState.kind === "close"
          ? "close"
          : `reduce ${Math.round((actionState.closePct ?? 0) * 100)}%`,
        actionState.target,
        "failed",
        {
          ...
          details: {
            ...
            ...(actionState.closePct != null
              ? { closePct: actionState.closePct }
              : {}),
    ...
    try {
      if (actionState.kind === "close") {
        ...
        await om.handleAction(action);
        await logOperatorAuditEntry(
          `reduce ${Math.round(closePct * 100)}%`,
          actionState.target,
          "submitted",
          {
            coin: pos.coin,
...
async function reportHandler(): Promise<string> {
  ...
  try {
    ...
    if (operatorEntries.length > 0) {
      const submitted = operatorEntries.filter(
        (entry) => entry.details.status === "submitted",
      ).length;
      const failed = operatorEntries.filter(
        (entry) => entry.details.status === "failed",
      ).length;
      lines.push(``, `*Operator Recent:*`);
      lines.push(
        `  ${esc(String(operatorEntries.length))} actions \\| ${esc(String(submitted))} submitted \\| ${esc(String(failed))} failed`,
      );
      for (const entry of operatorEntries.slice(0, 3)) {
        ...
        const target =
          typeof entry.details.target === "string"
            ...
            : (entry.coin ?? "unknown");
        const source =
          typeof entry.details.operatorSource === "string"
            ? entry.details.operatorSource.toUpperCase()
            : "MANUAL";
        lines.push(
    ...
    if (liveTraces.length > 0) {
      ...
      for (const trace of liveTraces) {
        const guardian =
          trace.roles.guardian != null
            ? formatRoleState(trace.roles.guardian.state)
            : "WAITING";
        const executor =
          trace.roles.executor != null
            ? formatRoleState(trace.roles.executor.state)
            : "IDLE";
        lines.push(
...
function getBriefingTraceCallback(target: {
  positionId?: string | null;
  coin?: string | null;
}): string {
  const normalizedCoin = target.coin?.trim().toUpperCase() ?? null;
...
export function getBriefingReplyMarkup(context: BriefingReplyContext = {}): {
  ...
  const healthButtons =
    (context.healthTarget?.positionId != null &&
      context.healthTarget.positionId.length > 0) ||
    (healthCoin != null && healthCoin.length > 0)
      ? (() => {
          const healthTraceCallback = getBriefingTraceCallback(
            context.healthTarget ?? {},
          );
          const healthOperatorCallback =
            ...
            context.healthTarget.positionId.length > 0
              ...
              : healthCoin != null && healthCoin.length > 0
                ...
                : "c:operator";
          const preferredHealthAction =
            context.preferredHealthAction ?? "trace";
          const healthButtonLabels = {
            trace: context.healthButtonLabels?.trace ?? "⚠️ Health Trace",
            operator:
              context.healthButtonLabels?.operator ?? "⚠️ Health Operator",
          };
          const orderedButtons =
            preferredHealthAction === "operator"
              ? [
                  {
                    text: healthButtonLabels.operator,
                    callback_data: healthOperatorCallback,
                  },
                  {
                    text: healthButtonLabels.trace,
                    callback_data: healthTraceCallback,
                  },
                ]
              : [
                  {
  ...
  const bucketButtons = (context.buckets ?? [])
    .filter((bucket) => {
      if (bucket.positionId != null && bucket.positionId.length > 0)
        return true;
...
export function registerBuiltinCommands(): void {
  ...
  registerCommand({
    name: "menu",
    description: "Menu + inline buttons",
    handler: menuHandler,
  });
  registerCommand({
    name: "status",
    description: "Agent state, health, uptime",
    handler: statusHandler,
  });
  registerCommand({
    name: "trace",
    description: "Latest decision trace or lookup by setup/position",
    handler: traceHandler,
  });
  registerCommand({
    name: "operator",
    description: "Recent manual operator interventions",
  ...
  registerCommand({
    name: "pnl",
    description: "PnL summary (daily/weekly/monthly)",
    handler: pnlHandler,
  });
  registerCommand({
    name: "pause",
    description: "Pause agent or coin (/pause BTC 4h)",
  ...
  registerCommand({
    name: "risk",
    description: "Risk dashboard (PnL, CB, losses)",
    handler: riskHandler,
  });
  registerCommand({
    name: "closeall",
    description: "Emergency close all (requires /confirm)",
    handler: closeallHandler,
  });
  registerCommand({
    name: "confirm",
    description: "Confirm pending close-all or operator action",
  ...
  registerCommand({
    name: "mode",
    description: "Show current trading mode (DEMO / LIVE)",
    handler: modeHandler,
  });
}
