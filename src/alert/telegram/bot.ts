...
const briefingMessageCache = new Map<
  string,
  {
    text: string;
...
>();
const briefingRefreshJobs = new Map<
  string,
  {
...
function resolveBotConfig(): BotConfig | null {
  ...
  if (Number.isNaN(chatId)) {
    log.warn(
      "bot",
      `Invalid ${TELEGRAM.chatIdEnv}: "${chatIdStr}" — must be a number`,
    );
    return null;
  }
...
async function registerBotCommands(
  ...
  const cmds = getCommands().map((c) => ({
    command: c.name,
    description:
      c.description.length > 96
        ? `${c.description.slice(0, 93)}...`
        : c.description,
  }));
  const ok = await postBotApi(
    config,
    "setMyCommands",
    { commands: cmds },
    fetchFn,
...
async function getUpdates(
  ...
  const res = await fetchFn(url, {
    ...
    signal: AbortSignal.timeout(
      (TELEGRAM_BOT.pollingTimeoutSec + TELEGRAM_BOT.pollingExtraTimeoutSec) *
        1000,
    ),
  });
...
function formatBriefingAttention(trace: DecisionTrace): {
  level: string;
  summary: string;
} {
...
function summarizeBriefingIncident(
  ...
  const failed = Math.max(
    ...
    incident.transitions.filter((transition) => transition.outcome === "failed")
      .length,
  );
  const coalesced = Math.max(
    snapshot.coalesced,
    incident.transitions.filter(
      (transition) => transition.outcome === "coalesced",
    ).length,
  );
  const skippedIdentical = Math.max(
    snapshot.skippedIdentical,
    incident.transitions.filter(
      (transition) => transition.outcome === "skipped_identical",
    ).length,
  );
  ...
  if (failed > 0 && failed >= coalesced) {
    return {
      ...
      recommendedAction:
        "Open Health Operator first, then refresh the briefing after the failing action path is clear.",
  ...
  if (coalesced >= 2) {
    return {
      ...
      recommendedAction:
        "Open Health Trace first, then use Health Operator only if the case still needs manual intervention.",
  ...
  if (skippedIdentical >= 1) {
    return {
      ...
      recommendedAction:
        "Open Health Trace first, wait for the case to change state, then refresh the briefing once.",
      preferredHealthAction: "trace",
      healthButtonLabels: {
        trace: "⚠️ Inspect Health Trace",
        operator: "🧾 Check Operator",
  ...
  return {
    ...
    recommendedAction:
      "Open Health Trace first, then use Health Operator if the target needs manual intervention.",
    preferredHealthAction: "trace",
    healthButtonLabels: {
      trace: "⚠️ Inspect Health Trace",
      operator: "🧾 Check Operator",
...
function classifyBriefingBucket(trace: DecisionTrace): {
  label: string;
  priority: number;
} {
...
function getLiveOversightTraces(): DecisionTrace[] {
  return [...getDecisionTraces()]
    .sort((a, b) => {
      const severityDelta =
        computeBriefingSeverity(b) - computeBriefingSeverity(a);
...
function resolveBriefingFocus(liveTraces: DecisionTrace[]): {
  positionId?: string;
  coin?: string;
} {
  const hottestPosition = liveTraces.find(
    (trace) => trace.outcome.positionId != null,
  );
  ...
  const firstTracked = getPositionMonitor()
    .getPositions()
    .values()
    .next().value;
...
async function buildScheduledBriefing(
  ...
  const html = formatScheduledBriefingHtml(
    title,
    {
      ...summary,
    ...
    },
    {
      ...
      incident:
        leadIncident != null
          ? (() => {
              const summary = summarizeBriefingIncident(
                leadIncident,
                healthSnapshot,
              );
              return {
      ...
      operatorRecent: {
        ...
        submitted: operatorEntries.filter(
          (entry) => entry.details.status === "submitted",
        ).length,
        failed: operatorEntries.filter(
...
function inferBriefingKind(
  text: string | undefined,
): "morning" | "evening" | "live" {
...
function isBriefingKind(
  value: string,
): value is "morning" | "evening" | "live" {
...
async function buildBriefingRefreshPayload(
  ...
  const yesterdayYmd = getYmdInTimeZone(
  ...
  const incidentSummary =
    leadIncident != null
      ? summarizeBriefingIncident(leadIncident, healthSnapshot)
      : null;
  ...
  return {
    ...
    context: {
      ...
      attention:
        leadTrace != null
          ? `${formatBriefingAttentionLabel(leadTrace)} ${leadTrace.outcome.action.toUpperCase()}`
          : null,
    },
  };
}
...
function shouldSkipBriefingEdit(
  ...
  return (
    previousPayload.text === html &&
    previousPayload.replyMarkupJson === JSON.stringify(replyMarkup)
  );
}
...
function formatBriefingHealthSummary(
  snapshot: BriefingRefreshHealthSnapshot,
): string {
  ...
  if (snapshot.failureStreak > 0)
    pieces.push(`${snapshot.failureStreak} fail streak`);
  if (snapshot.coalescedStreak > 0)
    pieces.push(`${snapshot.coalescedStreak} coalesced streak`);
...
function formatBriefingHealthFollowupIntro(
  ...
  const contextLines: string[] = [`Target: ${escapeMarkdownV2(targetLabel)}.`];
  if (isCoinMatch && healthTarget.attention != null) {
    contextLines.push(
      `Attention: ${escapeMarkdownV2(healthTarget.attention)}.`,
    );
  }

  if (
    leadIncident == null ||
    !matchesBriefingHealthTarget(target, healthTarget)
  ) {
    const title = mode === "trace" ? "🧭 *Trace Focus*" : "🧭 *Operator Focus*";
    return [title, ...contextLines].join("\n");
  }

  const incidentSummary = summarizeBriefingIncident(
    leadIncident,
    healthSnapshot,
  );
  ...
  const title =
    leadIncident.status === "active"
      ? mode === "trace"
        ? "⚠️ *Health Trace Focus*"
        : "⚠️ *Health Operator Focus*"
      : mode === "trace"
        ? "✅ *Recovered Trace Check*"
        : "✅ *Recovered Operator Check*";
  const lead =
    mode === "trace"
  ...
  return [title, lead, ...contextLines, hint].join("\n");
}

function prependBriefingHealthIntro(
  reply: string,
  intro: string | null,
): string {
  ...
  const isBasicFocus =
    intro.startsWith("🧭 *Trace Focus*") ||
    intro.startsWith("🧭 *Operator Focus*");
  if (isBasicFocus) return reply;

  const isIncidentFocused =
    intro.includes("Health Trace Focus") ||
...
async function announceBriefingHealthTransition(
  ...
  try {
    ...
    if (ok) {
      ...
      log.warn(
        "bot",
        `Briefing health alert failed (${transition.from} → ${transition.to})`,
      );
    }
  } catch (err) {
...
async function refreshBriefingMessage(
  ...
  if (existing != null) {
    ...
    existing.timer = setTimeout(async () => {
      ...
      if (job == null) return;
      try {
        const { html, replyMarkup, context } =
          await buildBriefingRefreshPayload(job.latest.kind, job.latest.now);
  ...
  const job = {
    timer: setTimeout(async () => {
      ...
      try {
        const { html, replyMarkup, context } =
          await buildBriefingRefreshPayload(
            currentJob.latest.kind,
            currentJob.latest.now,
          );
...
async function tickDayReports(
  ...
  if (h === 0 && m <= 3 && lastMorningReportForYmd !== todayYmd) {
    ...
    try {
      ...
      const { html, focus } = await buildScheduledBriefing(
        "Đầu ngày — hôm qua",
        summary,
      );
...
function startDayReportScheduler(config: BotConfig, fetchFn: FetchFn): void {
  ...
  log.info(
    "bot",
    `Day report scheduler (${TELEGRAM_BOT.reportTimezone}, every 45s tick)`,
  );
}
...
async function routeCallback(
  ...
  if (
    rest.startsWith("trace_close:") ||
    rest.startsWith("trace_reduce25:") ||
    rest.startsWith("trace_reduce50:")
  ) {
    const match = rest.match(
      /^(trace_close|trace_reduce25|trace_reduce50):(.+?)(?::b:(\d+):(morning|evening|live))?$/,
    );
    if (match == null) {
      await sendTelegramAlert(`Command failed\\. Check logs\\.`, fetchFn);
      return;
    ...
    try {
      const briefingSource =
        briefingMessageIdRaw != null &&
        briefingKindRaw != null &&
        isBriefingKind(briefingKindRaw)
          ? {
      ...
      const reply =
        briefingSource != null
          ? await requestRemoteOperatorActionWithContext(
              operatorArgs,
              chatId,
              briefingSource,
            )
      ...
      await sendMarkdownReply(
        reply,
        fetchFn,
        getPendingOperatorActionReplyMarkup(chatId) ?? undefined,
      );
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : String(err);
      log.error("bot", `Callback c:${rest} failed: ${errMsg}`);
  ...
  if (rest.startsWith("trace_refresh:")) {
    const match = rest.match(
      /^trace_refresh:(.+?)(?::b:(\d+):(morning|evening|live))?$/,
    );
    if (match == null) {
      await sendTelegramAlert(`Command failed\\. Check logs\\.`, fetchFn);
      return;
    ...
    try {
      const reply = await executeCommandByName(
        "trace",
        `position ${positionId}`,
        chatId,
      );
  ...
  if (rest.startsWith("briefing_refresh:")) {
    ...
    const kind = isBriefingKind(rawKind)
      ? rawKind
      : inferBriefingKind(cq.message?.text);
    const messageId = cq.message?.message_id;
  ...
  if (rest.startsWith("briefing_operator_coin:")) {
    const coin = rest
      .slice("briefing_operator_coin:".length)
      .trim()
      .toUpperCase();
    try {
      const briefingSource = getCallbackBriefingSource(cq);
...
async function routeUpdate(
  ...
  if (msg.chat.id !== config.chatId) {
    log.warn(
      "bot",
      `Unauthorized chat ID ${msg.chat.id} from ${msg.from?.username ?? "unknown"} — dropping`,
    );
    return;
  }
  ...
  try {
    ...
    const tracePositionId =
      parsed.name === "trace" ? parseTracePositionArgs(parsed.args) : null;
    const replyMarkup =
    ...
    await sendTelegramAlert(
      `Command /${parsed.name} failed\\. Check logs\\.`,
      fetchFn,
    );
  }
}
...
export async function startBot(
  ...
  if (!config) {
    log.warn(
      "bot",
      `Bot disabled — set ${TELEGRAM.tokenEnv} and ${TELEGRAM.chatIdEnv} env vars`,
    );
    return;
  }
  ...
  const cmdNames = getCommands()
    .map((c) => `/${c.name}`)
    .join(", ");
  log.info(
    "bot",
    `Telegram bot started (long-polling, ${getCommands().length} commands: ${cmdNames})`,
  );
