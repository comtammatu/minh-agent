...
describe("parseCommand", () => {
  ...
  it("parses command with args", () => {
    expect(parseCommand("/pause BTC 4h")).toEqual({
      name: "pause",
      args: "BTC 4h",
    });
  });
  ...
  it("strips @BotName with args", () => {
    expect(parseCommand("/status@MinhBot details")).toEqual({
      name: "status",
      args: "details",
    });
  });
...
describe("getUpdates", () => {
  ...
  it("returns updates from API response", async () => {
    ...
    const mockFetch: typeof fetch = async () => {
      const body: TelegramApiResponse<TelegramUpdate[]> = {
        ok: true,
        result: mockUpdates,
      };
  ...
  it("throws on HTTP error", async () => {
    ...
    expect(getUpdates(config, mockFetch)).rejects.toThrow(
      "getUpdates HTTP 401",
    );
  });
