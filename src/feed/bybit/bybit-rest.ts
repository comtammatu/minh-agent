 ...
 */
export function __setBybitRestTestDeps(deps: {
...
export async function loadBybitFundingRates(): Promise<void> {
  try {
    ...
    if (resp.retCode !== 0) {
      log.warn(
        "bybit-funding",
        `getTickers failed: retCode=${resp.retCode} ${resp.retMsg}`,
      );
      return;
    }
    fundingRates.clear();
...
export async function fetchBybitCandles(
  ...
  const bybitInterval = BYBIT_INTERVAL_MAP[interval] as
    | KlineIntervalV3
    | undefined;
  if (!bybitInterval) {
    log.warn(
      "bybit-backfill",
      `${coin} ${interval}: no interval mapping — skipping`,
    );
    return null;
  }
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (response.retCode !== 0) {
        ...
        const isRateLimit =
          response.retCode === 10006 || response.retMsg?.includes("rate limit");
        if (isRateLimit) {
          rateLimitHits++;
          if (rateLimitHits >= MAX_RATE_LIMIT_RETRIES) {
            log.warn(
              "bybit-backfill",
              `${coin} ${interval}: rate limited ${rateLimitHits}x — giving up`,
            );
            return null;
          }
          log.warn(
            "bybit-backfill",
            `${coin} ${interval}: rate limited (${rateLimitHits}/${MAX_RATE_LIMIT_RETRIES}) — waiting 2s`,
          );
        ...
        if (attempt > maxRetries) {
          log.warn(
            "bybit-backfill",
            `${coin} ${interval}: API error after ${maxRetries} retries — ${msg}`,
          );
          return null;
        }
        ...
        log.warn(
          "bybit-backfill",
          `${coin} ${interval}: API error (attempt ${attempt}/${maxRetries}): ${msg}`,
        );
      ...
      log.warn(
        "bybit-backfill",
        `${coin} ${interval}: error (attempt ${attempt}/${maxRetries}): ${msg}`,
      );
      await new Promise((r) => setTimeout(r, delay));
    }
  }
...
export async function fetchBybitCandlesBatched(
  ...
  for (let i = 0; i < batches; i++) {
    ...
    if (candles === null) {
      ...
      log.warn(
        "bybit-backfill",
        `${coin} ${interval}: batch ${i + 1}/${batches} failed — using ${all.length} candles`,
      );
      break;
    }
...
export async function backfillBybitCoins(
  ...
  if (skipped > 0) {
    log.info(
      "bybit-backfill",
      `skipping ${skipped} coin/TF pairs already loaded from PG`,
    );
  }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    async function runTask(
      ...
      try {
        ...
        const candles = await fetchBybitCandlesBatched(
          coin,
          interval,
          totalCount,
        );
        ...
        if (candles === null) {
          ...
          log.info(
            "bybit-backfill",
            `${coin} ${interval}: ${candles.length} candles`,
          );
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        log.error(
          "bybit-backfill",
          `${coin} ${interval}: unexpected error — ${msg}`,
        );
      }
    }
