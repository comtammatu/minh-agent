...
import { initDelta, applyDeltaUpdate, removeDelta } from '../trades.js'
  ...
  if (!wsClient) {
    ...
    wsClient.on('update', (data: { topic?: string; data?: unknown[] }) => {
      ...
      try {
        ...
        log.error('bybit-trades', `${coin}: parse error — ${err instanceof Error ? err.message : String(err)}`)
      }
    })

    wsClient.on('exception', (err: unknown) => {
      log.warn('bybit-trades', `WS exception: ${err instanceof Error ? err.message : String(err)}`)
    })
  }
  return wsClient
}
