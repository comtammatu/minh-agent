...
const oiStore = new Map<
  string,
  { current: AssetCtxSnapshot; previous: AssetCtxSnapshot | null }
>();
...
export function getOiDelta(coin: string): number | null {
  ...
  return (
    (entry.current.openInterest - entry.previous.openInterest) /
    entry.previous.openInterest
  );
}
...
export function hasDivergence(coin: string): boolean {
  ...
  return (
    Math.abs(markPrice - oraclePrice) / oraclePrice >
...
export async function startOiFeed(coins: string[]): Promise<void> {
  ...
  try {
    ...
    log.warn(
      "asset-ctx",
      `Failed to load perpDexs list: ${err instanceof Error ? err.message : err}`,
    );
    dexs = [];
  }
  ...
  for (const dex of selected) {
    try {
      ...
      dexCoinNames.set(
        dex,
        hip3Meta.universe.map((a) => a.name),
      );
      log.info(
        "asset-ctx",
        `Perp dex "${dex}": loaded ${hip3Meta.universe.length} coin names`,
      );
    } catch (err) {
      log.warn(
        "asset-ctx",
        `Failed to load HIP-3 names for dex "${dex}": ${err instanceof Error ? err.message : err}`,
      );
    }
  }
    ...
    try {
      ...
      for (const [dexId, ctxs] of event.ctxs) {
        ...
        for (let i = 0; i < ctxs.length; i++) {
          ...
          if (
            Number.isNaN(snapshot.openInterest) ||
            Number.isNaN(snapshot.markPrice) ||
            Number.isNaN(snapshot.oraclePrice)
          ) {
            continue;
          }
      ...
      log.error(
        "asset-ctx",
        `WS parse error — ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  });
