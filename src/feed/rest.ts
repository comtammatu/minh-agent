...
export function __setRestTestDeps(deps: {
  fetchCandlesBatched?: FetchCandlesBatchedFn;
}): void {
 ...
 */
export function backfillStartTime(
  interval: CandleInterval,
  count?: number,
): number {
  if (count === undefined)
    count = BACKFILL_CANDLE_COUNTS[interval] ?? BACKFILL_CANDLE_COUNT;
...
export async function probeCoins(
  ...
  if (failed.length > 0) {
    log.info(
      "probe",
      `${failed.length} coins unavailable: ${failed.join(", ")}`,
    );
  }
