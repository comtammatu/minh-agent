 ...
 *
 * Writes into the shared stores (asset-ctx, funding, orderbook) so
...
import { removeAssetCtx, writeAssetCtx } from "../asset-ctx.js";
import { removeFunding, writeFunding } from "../funding.js";
import { removeBook, writeBook } from "../orderbook.js";
...
function mergeDelta(
  state: TickerState,
  delta: Partial<Record<string, string>>,
): TickerState {
...
function flushToStores(coin: string, state: TickerState): void {
  ...
  const assetCtx: AssetCtxSnapshot = {
    ...
    openInterest:
      state.openInterestValue > 0
        ? state.openInterestValue
        : state.openInterest,
    funding: state.fundingRate,
    premium:
      state.markPrice > 0 && state.indexPrice > 0
  ...
  if (state.bid1Price > 0 && state.ask1Price > 0) {
    ...
    const imbalance =
      totalSize > 0 ? (state.bid1Size - state.ask1Size) / totalSize : 0;
