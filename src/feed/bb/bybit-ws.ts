...
function getClient(): WebsocketClient {
  if (!wsClient) {
    ...
    wsClient.on("update", (data: WSKlineEventV5) => {
      ...
      for (const candleData of data.data) {
        ...
        try {
          ...
          log.error(
            "bybit-ws",
            `parse error on ${topic}: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
    });
    ...
    wsClient.on("open", () => {
      ...
      log.info(
        "bybit-ws",
        `WS reconnected — re-subscribing ${topics.length} topics`,
      );
    ...
    wsClient.on("exception", (err: unknown) => {
      log.error(
        "bybit-ws",
        `WebSocket exception: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
  }
  return wsClient;
