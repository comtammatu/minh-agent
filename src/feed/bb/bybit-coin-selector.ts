...
import { type GetTickersParamsV5, RestClientV5 } from "bybit-api";
import {
...
export async function fetchBybitRankedCoins(
  ...
  try {
    ...
    for (const t of tickers) {
      ...
      if (Number.isNaN(oi) || oi <= 0) continue;
...
export async function fetchBybitTopCoins(
  ...
  if (ranked.length === 0) {
    log.warn(
      "bybit-coin-sel",
      `fetchBybitRankedCoins returned empty — using static fallback (${BYBIT_STATIC_COINS.length} coins)`,
    );
...
export function makeBybitFetchRankedFn(): () => Promise<string[]> {
  return async () => {
    ...
    if (ranked.length === 0) {
      log.warn(
        "bybit-coin-sel",
        "ranked fetch failed — returning static fallback",
      );
