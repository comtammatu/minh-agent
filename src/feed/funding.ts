...
import { FUNDING_HISTORY_HOURS, FUNDING_POLL_INTERVAL_MS } from "../config.js";
...
async function fetchFunding(coin: string): Promise<void> {
  ...
  try {
    ...
    const valid = snapshots.filter(
      (s) => !Number.isNaN(s.rate) && !Number.isNaN(s.premium),
    );
    fundingStore.set(coin, valid);
  } catch (err) {
    log.error(
      "funding",
      `${coin}: fetch error — ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
