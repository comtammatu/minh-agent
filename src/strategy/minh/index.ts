/**
 * minh strategy — ICT Multi-TF Drill-Down.
 *
...
import {
  ...
  MIN_CONFIDENCE,
  SL_WICK_ATR_MULT,
  MINH_1H_ALLOWED_COINS,
  MINH_1H_BOS_PENALTY,
  MINH_1H_MIN_ADX,
  MINH_1H_MIN_VOLUME_RATIO,
  MINH_4H_SWING_CONFIDENCE_BASE,
  MINH_4H_SWING_ENABLED,
  MINH_4H_SWING_MIN_RR,
  MINH_4H_SWING_SL_ATR_BUFFER,
  MINH_5M_CONFIDENCE_BASE,
  MINH_5M_FVG_LOOKBACK,
  MINH_5M_MIN_RR,
  MINH_5M_MIN_SL_PCT,
  MINH_5M_REQUIRE_15M_CHOCH,
  MINH_5M_SL_ATR_BUFFER,
  MINH_15M_SCALP_CONFIDENCE_BASE,
  MINH_15M_SCALP_ENABLED,
  MINH_15M_SCALP_MIN_RR,
  MINH_15M_SCALP_SL_ATR_BUFFER,
  MINH_AMD_ACCUMULATION_END_UTC,
  MINH_AMD_ACCUMULATION_START_UTC,
  MINH_AMD_CONFIDENCE_BASE,
  MINH_AMD_ENABLED,
  MINH_AMD_JUDAS_BONUS,
  MINH_AMD_MANIPULATION_WINDOWS,
  MINH_AMD_MIN_RANGE_BARS,
  MINH_AMD_MIN_RR,
  MINH_AMD_SL_ATR_BUFFER,
  MINH_BREAK_LOOKBACK,
  MINH_COIN_BLACKLIST,
  MINH_CONFIRMED_POI_MAX,
  MINH_CONFIRMED_POI_TTL_MS,
  MINH_DEDUP_BARS,
  MINH_DRILLDOWN_CHOCH_BONUS,
  MINH_DRILLDOWN_MAX_POIS,
  MINH_HTF_POI_TTL_MS,
  MINH_ICT_BREAKER_BLOCK_BONUS,
  MINH_ICT_BREAKER_BLOCK_ENABLED,
  MINH_ICT_DISPLACEMENT_BODY_ATR,
  MINH_ICT_HTF_ALIGNED_BONUS,
  MINH_ICT_HTF_ALIGNMENT,
  MINH_ICT_HTF_COUNTER_PENALTY,
  MINH_ICT_INVERSION_FVG_BONUS,
  MINH_ICT_INVERSION_FVG_ENABLED,
  MINH_ICT_KILLZONE_ENABLED,
  MINH_ICT_KILLZONE_PENALTY,
  MINH_ICT_KILLZONES,
  MINH_ICT_LIQUIDITY_POOL_TP_BONUS,
  MINH_ICT_OTE_BONUS,
  MINH_ICT_OTE_FILTER,
  MINH_ICT_REQUIRE_SWEEP_FOR_THROUGH,
  MINH_LIQUIDATION_CONFIDENCE_MULT,
  MINH_LIQUIDATION_VOLUME_RATIO,
  MINH_LIQUIDATION_WICK_ATR_MULT,
  MINH_LTF_CHOCH_LOOKBACK,
  MINH_MIN_BODY_RATIO,
  MINH_MIN_RR,
  MINH_MIN_ZONE_STRENGTH,
  MINH_PRICE_TOLERANCE_ATR_MULT,
  MINH_SKIP_INTERVALS,
  MINH_WEEKEND_CONFIDENCE_MULT,
  MINH_WEEKEND_VOLUME_RATIO_THRESHOLD,
...
const smc1hAllowedCoinSet: ReadonlySet<string> = new Set(MINH_1H_ALLOWED_COINS);
...
function getKillzoneBonus(timestampMs: number): {
  ...
  for (const kz of MINH_ICT_KILLZONES) {
...
function isLiquidationCascade(
  ...
  return (
    wickSize > atrVal * MINH_LIQUIDATION_WICK_ATR_MULT &&
    !Number.isNaN(volRatio) &&
    volRatio > MINH_LIQUIDATION_VOLUME_RATIO
  );
}
...
function getWeekendMultiplier(timestampMs: number, volRatio: number): number {
  ...
  if (Number.isNaN(volRatio) || volRatio >= MINH_WEEKEND_VOLUME_RATIO_THRESHOLD)
    return 1.0;
  return MINH_WEEKEND_CONFIDENCE_MULT;
}
...
function isDuplicateSignal(
  ...
  dedupBars: number = MINH_DEDUP_BARS,
): boolean {
...
export class MinhStrategy implements SetupGenerator {
  readonly id = "minh";
  readonly name = "SMC + S&D Zone Bounce (ICT)";
  readonly patternTypes: ReadonlyArray<PatternType> = ["minh"];
  ...
  scan(
    ...
    if (MINH_SKIP_INTERVALS.includes(interval)) return null;
    if (MINH_COIN_BLACKLIST.includes(coin)) return null;
  ...
  private scan4hPOI(
    ...
    const tol = atrVal * MINH_PRICE_TOLERANCE_ATR_MULT;
    ...
    for (let i = breaks.length - 1; i >= 0; i--) {
      const b = breaks[i]!;
      if (idx - b.index <= MINH_BREAK_LOOKBACK) {
        recentBreak = b;
        break;
      }
    }
    ...
    const pushZoneCandidate = (
      ...
      if (strength < MINH_MIN_ZONE_STRENGTH) return;
      if (
        zones.length === MINH_DRILLDOWN_MAX_POIS &&
        (zones[zones.length - 1]?.strength ?? 0) >= strength
      )
        return;

      let insertAt = zones.length;
      while (insertAt > 0 && (zones[insertAt - 1]?.strength ?? 0) < strength) {
        insertAt--;
      }
      ...
      if (zones.length > MINH_DRILLDOWN_MAX_POIS) {
        zones.length = MINH_DRILLDOWN_MAX_POIS;
      }
    };
    ...
    const weakestZoneStrength =
      zones.length === MINH_DRILLDOWN_MAX_POIS
        ? (zones[zones.length - 1]?.strength ?? Number.POSITIVE_INFINITY)
        : -Infinity;

    if (MINH_ICT_BREAKER_BLOCK_ENABLED && weakestZoneStrength < 0.85) {
    ...
    if (MINH_ICT_INVERSION_FVG_ENABLED && weakestZoneStrength < 0.75) {
    ...
    for (const p of htfPOIs.get(coin) ?? []) {
      if (nowMs - p.createdAtMs < MINH_HTF_POI_TTL_MS) activePois.push(p);
    }
    ...
    if (activePois.length > MINH_DRILLDOWN_MAX_POIS) {
      activePois.splice(0, activePois.length - MINH_DRILLDOWN_MAX_POIS);
    }
    htfPOIs.set(coin, activePois);
    diag.scan4h_pois_registered += zones.length;

    // ── 4h SWING SIGNAL: emit if price is currently AT a zone ─────────────
    if (MINH_4H_SWING_ENABLED) {
      ...
      const kz = MINH_ICT_KILLZONE_ENABLED
      ...
      for (const z of zones) {
        ...
        if (candleRange > 0 && bodySize / candleRange < MINH_MIN_BODY_RATIO)
          continue;
        ...
        const sl =
          side === "long"
            ? z.bottom - atrVal * MINH_4H_SWING_SL_ATR_BUFFER * slMult
            : z.top + atrVal * MINH_4H_SWING_SL_ATR_BUFFER * slMult;
        ...
        if (Math.abs(tp1 - entry) / risk < MINH_4H_SWING_MIN_RR) continue;

        // Confidence
        let confidence =
          strategyParams?.MINH_DRILLDOWN_CONFIDENCE_BASE ??
          MINH_4H_SWING_CONFIDENCE_BASE;
        if (recentBreak.kind === "choch") confidence += 0.1;
        if (z.origin === "breaker-block")
          confidence += MINH_ICT_BREAKER_BLOCK_BONUS;
        if (z.origin === "inversion-fvg")
          confidence += MINH_ICT_INVERSION_FVG_BONUS;
        if (z.strength > 0.6) confidence += 0.05;
        if (z.strength > 0.8) confidence += 0.05;
        if (hasDisplacement) confidence += 0.08;

        confidence += kz.inKillzone ? kz.bonus : -MINH_ICT_KILLZONE_PENALTY;
        ...
        if (isLiquidationCascade(candle, atrVal, volRatioVal))
          confidence *= MINH_LIQUIDATION_CONFIDENCE_MULT;
  ...
  private scan15mConfirm(
    ...
    if (!pois || pois.length === 0) {
      ...
      for (const poi of pois) {
        if (nowMs - poi.createdAtMs < MINH_HTF_POI_TTL_MS) activePOIs.push(poi);
        else expiredCount++;
      }
      ...
      for (const poi of activePOIs) {
        ...
        for (const confirmed of existing) {
          if (nowMs - confirmed.confirmedAtMs < MINH_CONFIRMED_POI_TTL_MS) {
            pool.push(confirmed);
          }
        }
        ...
        pool.push(fresh);
        if (pool.length > MINH_CONFIRMED_POI_MAX) {
          pool.splice(0, pool.length - MINH_CONFIRMED_POI_MAX);
        }
    ...
    if (!MINH_AMD_ENABLED) return null;

    // Check if current time is within a manipulation window
    const hourUTC = utcHour(nowMs);
    let activeWindow: (typeof MINH_AMD_MANIPULATION_WINDOWS)[number] | undefined;
    for (const w of MINH_AMD_MANIPULATION_WINDOWS) {
      const inWindow =
        w.start <= w.end
    ...
    if (!range || range.barCount < MINH_AMD_MIN_RANGE_BARS) return null;
    ...
    const sl =
      side === "long"
        ? judas.sweepLevel - atrVal * MINH_AMD_SL_ATR_BUFFER * slMult
        : judas.sweepLevel + atrVal * MINH_AMD_SL_ATR_BUFFER * slMult;
    ...
    // Floor: min R:R
    const minTp = entry + dir * risk * MINH_AMD_MIN_RR;
    if ((side === "long" && tp1 < minTp) || (side === "short" && tp1 > minTp))
      tp1 = minTp;

    const reward = Math.abs(tp1 - entry);
    if (reward / risk < MINH_AMD_MIN_RR) return null;

    // Confidence
    let confidence =
      strategyParams?.MINH_DRILLDOWN_CONFIDENCE_BASE ?? MINH_AMD_CONFIDENCE_BASE;
    confidence += MINH_AMD_JUDAS_BONUS;
    if (confirmBreak.kind === "choch") confidence += MINH_DRILLDOWN_CHOCH_BONUS;
    if (
      isDisplacementCandle(candles, idx, atrVal, MINH_ICT_DISPLACEMENT_BODY_ATR)
    )
      confidence += 0.08;
  ...
  private emit15mScalpSignal(
    ...
    if (side === "long") {
      ...
      sl = swingLow - atrVal * MINH_15M_SCALP_SL_ATR_BUFFER * slMult;
    } else {
      let highsFound = 0;
      ...
      sl = swingHigh + atrVal * MINH_15M_SCALP_SL_ATR_BUFFER * slMult;
    }
    const risk = Math.abs(entry - sl);
    if (risk <= 0 || risk / entry > MAX_TRADE_SL_PCT) return null;
    ...
    if (Math.abs(tp1 - entry) / risk < MINH_15M_SCALP_MIN_RR) return null;

    // Confidence
    let confidence =
      strategyParams?.MINH_DRILLDOWN_CONFIDENCE_BASE ??
      MINH_15M_SCALP_CONFIDENCE_BASE;
    if (poi.ltfBreakKind === "choch") confidence += MINH_DRILLDOWN_CHOCH_BONUS; // 15m CHoCH > BOS
  ...
  private scan5mMicroEntry(
    ...
    for (const poi of pool) {
      if (nowMs - poi.confirmedAtMs < MINH_CONFIRMED_POI_TTL_MS) {
        active.push(poi);
      }
    }
    if (active.length === 0) {
    ...
    for (let i = Math.max(2, idx - MINH_5M_FVG_LOOKBACK); i <= idx; i++) {
    ...
    if (side === "long") {
      ...
      sl = swingLow - atrVal * MINH_5M_SL_ATR_BUFFER * slMult;
    } else {
      let highsFound = 0;
      let swingHigh = candle.h;
      ...
      sl = swingHigh + atrVal * MINH_5M_SL_ATR_BUFFER * slMult;
    }
    ...
    if (slPct < MINH_5M_MIN_SL_PCT) {
      diag.scan5m_sl_too_tight++;
      return null;
    }

    // P3-B: Require 15m CHoCH confirmation (not just BOS)
    if (MINH_5M_REQUIRE_15M_CHOCH && bestPOI.ltfBreakKind !== "choch") {
    ...
    if (
      ...
      tp2 = targets.tp2;
      // Ensure minimum R:R
      const minTp = entry + dir * risk * MINH_5M_MIN_RR;
      if ((side === "long" && tp1 < minTp) || (side === "short" && tp1 > minTp))
        tp1 = minTp;
    } else {
      tp1 = entry + dir * risk * MINH_5M_MIN_RR;
      tp2 = entry + dir * risk * (MINH_5M_MIN_RR + 3);
    }

    // R:R check
    const reward = Math.abs(tp1 - entry);
    if (reward / risk < MINH_5M_MIN_RR) {
    ...
    // ── D. CONFIDENCE ──────────────────────────────────────────────────
    let confidence =
      strategyParams?.MINH_DRILLDOWN_CONFIDENCE_BASE ?? MINH_5M_CONFIDENCE_BASE;
    ...
    if (bestPOI.ltfBreakKind === "choch")
      confidence += MINH_DRILLDOWN_CHOCH_BONUS;
  ...
  private scan1hSameTF(
    ...
    if (!Number.isNaN(volRatio) && volRatio < MINH_1H_MIN_VOLUME_RATIO)
      return null;
    const tol = atrVal * MINH_PRICE_TOLERANCE_ATR_MULT;
    ...
    for (let i = breaks.length - 1; i >= 0; i--) {
      ...
      if (idx - candidate.index <= MINH_BREAK_LOOKBACK) {
    ...
    if (
      candleRange > 0 &&
      Math.abs(candle.c - candle.o) / candleRange < MINH_MIN_BODY_RATIO
    )
      return null;
    ...
    const tryAcceptZone = (z: KeyZone): boolean => {
      ...
      if (z.strength < MINH_MIN_ZONE_STRENGTH) return false;
      const prox = isAtZone(candle, z, atrVal);
    ...
    if (!bestZone && MINH_ICT_BREAKER_BLOCK_ENABLED) {
    ...
    if (!bestZone && MINH_ICT_INVERSION_FVG_ENABLED) {
    ...
    if (side === "long") {
      ...
      if (we && ca && hasDisplacement && bc) {
        ...
        if (!MINH_ICT_REQUIRE_SWEEP_FOR_THROUGH) {
          isBounce = true;
          bounceQuality = "sweep";
    ...
    if (!Number.isNaN(adxVal) && adxVal < MINH_1H_MIN_ADX) return null;

    // Confidence (base tunable via MINH_1H_CONFIDENCE_BASE optimizer param)
    const _base1h = strategyParams?.MINH_1H_CONFIDENCE_BASE;
    let confidence =
      typeof _base1h === "number" && Number.isFinite(_base1h) && _base1h > 0
        ? _base1h
        : 0.65;
    if (recentBreak.kind === "choch") confidence += 0.1;
    if (recentBreak.kind === "bos") confidence -= MINH_1H_BOS_PENALTY;
    ...
    if (
      ...
      confidence += 0.05;
    if (htfAligned) confidence += MINH_ICT_HTF_ALIGNED_BONUS;
    if (htfOpposed) confidence -= MINH_ICT_HTF_COUNTER_PENALTY;
    if (inOTE) confidence += MINH_ICT_OTE_BONUS;
    ...
    if (
      ...
      Math.abs(tp1 - entry) / riskAmt <
        (strategyParams?.MINH_MIN_RR ?? MINH_MIN_RR)
    )
      return null;
    ...
    if (tpDistance > 0) {
      ...
      if (
        ...
        confidence += MINH_ICT_LIQUIDITY_POOL_TP_BONUS;
      }
    }
