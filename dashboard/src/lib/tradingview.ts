...
export function createTradingViewDatafeed(): TradingViewDatafeed {
  return {
    ...
    resolveSymbol(symbolName, onResolve, onError) {
      void fetchSymbolInfo(symbolName)
        .then(onResolve)
        .catch((err) =>
    ...
    getBars(symbolInfo, resolution, periodParams, onHistory, onError) {
      ...
      void fetchJson<{ bars: ChartBar[]; noData: boolean }>(
    ...
    getMarks(symbolInfo, from, to, onData) {
      void fetchJson<{ marks: ChartMark[] }>(
        `/api/chart/overlays?ticker=${encodeURIComponent(symbolInfo.ticker)}`,
      )
        .then((payload) =>
          onData(
            payload.marks.filter(
              (mark) => mark.time >= from && mark.time <= to,
            ),
          ),
        )
        .catch(() => onData([]));
