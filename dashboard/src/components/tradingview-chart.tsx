...
import { createTradingViewDatafeed, ensureTradingView, type TradingViewChartApi, type TradingViewWidget } from '@/lib/tradingview'
...
async function syncHorizontalLines(chart: TradingViewChartApi, lines: ChartLine[], entities: Array<string | number>): Promise<Array<string | number>> {
...
export function TradingViewChart({ ticker, resolution, showMarks, showLines }: TradingViewChartProps) {
  ...
  }, [isReady, overlays.data?.marks, showMarks])
  ...
  useEffect(() => {
    ...
    void syncHorizontalLines(chart, lines, lineEntitiesRef.current).then(nextEntities => {
  ...
  if (!ticker) {
    return (
      <Alert>
        ...
        <AlertDescription>Choose a coin from the watchlist to mount the TradingView chart.</AlertDescription>
      </Alert>
    )
  }
