/**
 * Unit tests for MinhStrategy (SMC+S&D Zone Bounce).
 *
 * Tests verify:
...
describe("MinhStrategy", () => {
  ...
  test("patternTypes includes minh", () => {
    expect(strategy.patternTypes).toEqual(["minh"]);
  });
  ...
  test("returns null when idx < MIN_CANDLES_FOR_SCAN", () => {
    ...
    const result = strategy.scan(
      "BTC",
      "1h",
      candles,
      MIN_CANDLES_FOR_SCAN - 1,
    );
  ...
  test("dedup blocks consecutive signals on same coin/interval", () => {
    ...
    // If we got 2+ signals, check they're spaced by at least MINH_DEDUP_BARS (5)
    if (signals.length >= 2) {
  ...
  test("different coins are independent for dedup", () => {
    ...
    for (let i = MIN_CANDLES_FOR_SCAN; i < candles.length - 1; i++) {
      ...
      if (btcSignal) {
        ...
        const _ethSignal = strategy.scan("ETH", "1h", candles, i);
...
describe("P2: isLiquidationCascade — confidence discount", () => {
  test("signal with extreme wick + volume spike has reduced confidence vs normal bar", () => {
    const strategy = new MinhStrategy();
