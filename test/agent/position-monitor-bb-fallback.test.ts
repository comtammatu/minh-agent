...
mock.module('../../src/execution/exchange-pool.js', () => ({
  getExchangePool: () => ({
    ...
    isMultiWallet: () => false,
    getShared: () => {
      throw new Error('getShared should not be called when pool is not initialized')
    },
    get: () => {
...
import { queryExchangePositions, getPositionMonitor, resetPositionMonitor } from '../../src/agent/position-monitor.js'
