...
import type { ComponentHealth, ComponentStatus } from '../../src/agent/types.js'
...
describe('self-healing', () => {
  ...
  describe('computeComponentStatus', () => {
    ...
    it('returns degraded at 3 consecutive errors', () => {
      expect(computeComponentStatus(health({ consecutiveErrors: 3 }), 60_000, now)).toBe('degraded')
    })

    it('returns critical at 5 consecutive errors', () => {
      expect(computeComponentStatus(health({ consecutiveErrors: 5 }), 60_000, now)).toBe('critical')
    })
    ...
    it('critical beats degraded (5 errors + stale)', () => {
      const badHealth = health({ consecutiveErrors: 5, lastSuccessAt: now - 120_000 })
  ...
  describe('computeOverallStatus', () => {
    ...
    it('critical when any critical', () => {
      expect(computeOverallStatus(['ok', 'degraded', 'critical'])).toBe('critical')
    })
  ...
  describe('HealthMonitor', () => {
    ...
    it('recordError increments consecutive errors', () => {
      ...
      expect(monitor.getComponentHealth('db').lastError).toBe('connection lost')
