...
import { sma, ema, atr, rsi, adx, volumeRatio, volumeTrend, detectRegime } from '../src/indicators/core.js'
import {
  compileKeyZones,
  detectBreakerBlocks,
...
describe('smc indicators', () => {
  ...
  it('detectLiquiditySweep finds bullish sweep when wick breaks recent low and closes back inside', () => {
    ...
    expect(detectLiquiditySweep(sweepCandles, 4, { lookback: 4, wickRatio: 0.4 })).toEqual({
      direction: 'bullish',
      level: 99,
    })
  })
  ...
  it('detectLiquiditySweep returns null when neither wick can satisfy the configured ratio', () => {
    ...
    expect(detectLiquiditySweep(noSweepCandles, 4, { lookback: 4, wickRatio: 0.4 })).toBeNull()
  })
  ...
  it('detectWyckoff matches the previous naive implementation across default and custom params', () => {
    ...
    for (let idx = 100; idx < candles.length; idx += 7) {
      expect(detectWyckoff(candles, idx)).toEqual(naiveDetectWyckoff(candles, idx))
      expect(detectWyckoff(candles, idx, { rangePeriod: 12, trendPeriod: 40 }))
        .toEqual(naiveDetectWyckoff(candles, idx, { rangePeriod: 12, trendPeriod: 40 }))
    }
  })
  ...
  it('detectVSA matches the previous naive implementation across default and custom lookbacks', () => {
    ...
    const naiveDetectVsa = (series: Candle[], idx: number, lookback: number) => {
      ...
      if (volR > 2.5 && spreadR > 1.2 && current.c < current.o && trendChange < 0) {
        signals.push({ type: 'stopping-volume', direction: 'bullish', strength: Math.min(volR / 4, 1), index: idx })
      }
      if (volR < 0.5 && spreadR < 0.6 && lowerWick > body && trendChange < 0) {
        signals.push({ type: 'test', direction: 'bullish', strength: Math.min((1 - volR) * 0.8, 1), index: idx })
      }
      if (volR < 0.4 && spreadR < 0.5 && current.c > current.o && trendChange < 0) {
        signals.push({ type: 'no-supply', direction: 'bullish', strength: Math.min((1 - volR) * 0.7, 1), index: idx })
      }
      if (volR > 3.0 && spreadR > 1.0 && current.c > current.o && bodyR > 0.6) {
        signals.push({ type: 'climax-buy', direction: 'bullish', strength: Math.min(volR / 5, 1), index: idx })
      }
      if (volR > 2.5 && spreadR > 1.2 && current.c > current.o && trendChange > 0) {
        signals.push({ type: 'climax-sell', direction: 'bearish', strength: Math.min(volR / 4, 1), index: idx })
      }
      if (volR > 1.5 && upperWick > body * 2 && current.c < current.o) {
        signals.push({ type: 'thrust', direction: 'bearish', strength: Math.min(volR / 3, 1), index: idx })
      }
      if (volR < 0.4 && spreadR < 0.5 && current.c < current.o && trendChange > 0) {
        signals.push({ type: 'no-demand', direction: 'bearish', strength: Math.min((1 - volR) * 0.7, 1), index: idx })
      }
    ...
    for (const lookback of [12, 20, 30]) {
      for (let idx = lookback; idx < candles.length; idx += 7) {
        expect(detectVSA(candles, idx, { lookback })).toEqual(naiveDetectVsa(candles, idx, lookback))
      }
    }
  })
  ...
  it('findConfirmingBreak matches the previous naive implementation for bullish direction', () => {
    ...
    const naiveFindConfirmingBreak = (): ReturnType<typeof findConfirmingBreak> => {
    ...
    expect(findConfirmingBreak(sample, 55, 12, 'bullish', 0.1)).toEqual(naiveFindConfirmingBreak())
  })
  ...
  it('findConfirmingBreak matches the previous naive implementation for bearish direction', () => {
    ...
    expect(findConfirmingBreak(sample, 55, 12, 'bearish', 0.1)).toEqual(naiveFindConfirmingBreak())
  })
  ...
  it('detectStructureBreaks matches the previous clustered-pivot implementation', () => {
    ...
    const naiveDetectStructureBreaks = (): ReturnType<typeof detectStructureBreaks> => {
      const tol = 0.1
      const minSpacing = 5
      ...
      if (c.c > prevHigh.price - tol && Math.abs(c.c - prevHigh.price) >= minPriceDist * 0.3) {
        ...
        breaks.push({ kind: isCHoCH ? 'choch' : 'bos', direction: 'bullish', level: prevHigh.price, index: 70 })
      }
      if (c.c < prevLow.price + tol && Math.abs(c.c - prevLow.price) >= minPriceDist * 0.3) {
        ...
        breaks.push({ kind: isCHoCH ? 'choch' : 'bos', direction: 'bearish', level: prevLow.price, index: 70 })
      }
      return breaks
    }

    expect(detectStructureBreaks(sample, 70, { tolerance: 0.1 })).toEqual(naiveDetectStructureBreaks())
  })
    ...
    const naiveFindPivots = (): ReturnType<typeof findPivots> => {
      ...
      for (let i = lookback; i <= upToIdx - lookback; i++) {
        ...
        for (let j = 1; j <= lookback; j++) {
          if (c.h < sample[i - j]!.h - tolerance || c.h < sample[i + j]!.h - tolerance) isHigh = false
          if (c.l > sample[i - j]!.l + tolerance || c.l > sample[i + j]!.l + tolerance) isLow = false
        }
  ...
  it('detectOrderBlocks matches the previous naive implementation including tested flags', () => {
    const candles: Candle[] = Array.from({ length: 120 }, (_, i) => {
      ...
      const impulse = i % 8 === 0 ? 2.4 : i % 6 === 0 ? -1.9 : Math.sin(i / 4.8) * 0.85
      return {
        t: i * 3_600_000,
        o: base + impulse * 0.18,
        h: base + impulse + 1.05,
    ...
    expect(detectOrderBlocks(candles, upToIdx, { lookback: 50 }))
      .toEqual(naiveDetectOrderBlocks(candles, upToIdx, { lookback: 50 }))
  })
  ...
  it('htfStructureBias matches fallback behavior when caller provides pivots', () => {
    ...
    const fallback = htfStructureBias(sample, upToIdx, { swingLookback, tolerance })
    const injected = htfStructureBias(sample, upToIdx, { swingLookback, tolerance, pivots })
  ...
  it('compileKeyZones matches fallback behavior when caller provides pivots and FVGs', () => {
    ...
    const injected = compileKeyZones(sample, upToIdx, tolerance, { pivots, fvgs })
...
describe('core smoke tests', () => {
  ...
  it('atr matches the previous naive implementation', () => {
    ...
    const naiveAtr = (series: Candle[], idx: number, period: number): number => {
    ...
    for (const period of [7, 14, 21, 30]) {
      for (let idx = period; idx < candles.length; idx += 5) {
        expect(atr(candles, idx, period)).toBeCloseTo(naiveAtr(candles, idx, period), 10)
      }
    }
  })
  ...
  it('adx matches the previous naive implementation without dx array reuse', () => {
    ...
    const naiveAdx = (series: Candle[], idx: number, period: number = 14): number => {
      ...
      for (let i = period + 1; i <= idx; i++) {
        ...
        tr14 = tr14 - tr14 / period + Math.max(c.h - c.l, Math.abs(c.h - p.c), Math.abs(c.l - p.c))
...
describe('smc pure behavior', () => {
  ...
  it('findLiquidityPools matches the reference naive implementation', () => {
    ...
    const naive = (kind: 'high' | 'low', type: 'bsl' | 'ssl') => {
      ...
      const pools: Array<{ level: number; type: 'bsl' | 'ssl'; count: number; index: number }> = []
    ...
    const actual = findLiquidityPools(candles, upToIdx, { tolerance, minCluster })
  ...
  it('findLiquidityPools matches fallback behavior when ATR is provided explicitly', () => {
    ...
    const fallback = findLiquidityPools(candles, upToIdx, { tolerance, minCluster })
    const injected = findLiquidityPools(candles, upToIdx, { tolerance, minCluster, atrValue })
  ...
  it('detectBreakerBlocks matches the reference naive implementation', () => {
    const candles: Candle[] = Array.from({ length: 100 }, (_, i) => {
      const base = 100 + i * 0.08
      const impulse = i % 9 === 0 ? 2.2 : i % 7 === 0 ? -1.8 : Math.sin(i / 5) * 0.9
      return {
        t: i * 3_600_000,
        o: base + impulse * 0.2,
        h: base + impulse + 1.0,
    ...
    for (const ob of obs) {
      ...
      for (let i = breaker.index + 1; i <= upToIdx; i++) {
        const c = candles[i]!
        if (breaker.type === 'demand' && c.c < breaker.bottom) { stillValid = false; break }
        if (breaker.type === 'supply' && c.c > breaker.top) { stillValid = false; break }
      }
      if (stillValid) expected.push(breaker)
    }

    expect(detectBreakerBlocks(candles, upToIdx, { lookback: 50 })).toEqual(expected)
  })

  it('detectInversionFVGs matches the reference naive implementation', () => {
    const candles: Candle[] = Array.from({ length: 90 }, (_, i) => {
      const base = 100 + i * 0.05
      const jump = i % 10 === 2 ? 4.0 : i % 10 === 5 ? -3.5 : Math.sin(i / 4) * 0.6
      return {
        t: i * 3_600_000,
        o: base + jump * 0.15,
        h: base + jump + 1.2,
    ...
    for (let i = 2; i <= upToIdx; i++) {
      ...
      let inversion: ReturnType<typeof detectInversionFVGs>[number] | null = null
      ...
      for (let j = inversion.index + 1; j <= upToIdx; j++) {
        const c = candles[j]!
        if (inversion.type === 'demand' && c.c < inversion.bottom) { stillValid = false; break }
        if (inversion.type === 'supply' && c.c > inversion.top) { stillValid = false; break }
      }
