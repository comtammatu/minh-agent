...
import {
  ...
  isInvalidated,
  setupId,
} from "../src/strategy/shared/invalidation.js";
...
describe("invalidation — TTL", () => {
  ...
  it("does not invalidate before TTL", () => {
    ...
    const candles: Candle[] = Array(5)
      .fill(null)
      .map(() => makeCandle({ c: 100 }));
...
describe("invalidation — minh zone", () => {
  it("long: invalidates when close below zoneBottom - ATR buffer", () => {
    ...
    const setup = makeSetup(
      "long",
      { zoneBottom, zoneTop: 102, atrAtEntry: atr },
      0,
    );
  ...
  it("short: invalidates when close above zoneTop + ATR buffer", () => {
    ...
    const setup = makeSetup(
      "short",
      { zoneBottom: 98, zoneTop, atrAtEntry: atr },
      0,
    );
