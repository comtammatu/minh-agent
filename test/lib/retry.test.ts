...
import { withRetry, calculateDelay, is503, is429, isRetryableExchangeError } from '../../src/lib/retry.js'
...
describe('retry', () => {
  ...
  describe('isRetryableExchangeError', () => {
    it('retries 503 errors', () => {
      expect(isRetryableExchangeError(new Error('503 Service Unavailable'))).toBe(true)
    })
    ...
    it('does NOT retry validation errors', () => {
      expect(isRetryableExchangeError(new Error('Unknown asset: FAKE'))).toBe(false)
      expect(isRetryableExchangeError(new Error('minimum order value'))).toBe(false)
      expect(isRetryableExchangeError(new Error('not initialized'))).toBe(false)
    })
