...
import type { BacktestConfig, StrategyParams } from '../../src/backtest/types.js'
import type { Candle, CandleInterval } from '../../src/types.js'
import { REGIME_MULTIPLIERS, MIN_CONFIDENCE, SMC_MIN_RR } from '../../src/config.js'
...
describe('StrategyParams type', () => {
  test('StrategyParams has all 6 fields', () => {
    const params: StrategyParams = {
      MIN_CONFIDENCE: 0.60,
      REGIME_MULT_COUNTER: 0.30,
      REGIME_MULT_NEUTRAL: 0.80,
      SMC_DRILLDOWN_CONFIDENCE_BASE: 0.70,
      SL_WICK_ATR_MULT: 0.5,
      SMC_MIN_RR: 2.0,
    }
    expect(params.MIN_CONFIDENCE).toBe(0.60)
    expect(params.REGIME_MULT_COUNTER).toBe(0.30)
    expect(params.REGIME_MULT_NEUTRAL).toBe(0.80)
    expect(params.SMC_DRILLDOWN_CONFIDENCE_BASE).toBe(0.70)
...
describe('MIN_CONFIDENCE override', () => {
  ...
  test('MIN_CONFIDENCE=0.01 produces more or equal trades vs default', () => {
    ...
    expect(lowConf.trades.length).toBeGreaterThanOrEqual(baseline.trades.length)
  })
})
...
describe('default equivalence', () => {
  test('undefined strategyParams = same results as no strategyParams', () => {
    ...
    expect(noParams.metrics.profitFactor).toBe(undefinedParams.metrics.profitFactor)
  })
  ...
  test('explicit default values = same results as no params', () => {
    ...
    expect(noParams.metrics.profitFactor).toBe(explicitDefaults.metrics.profitFactor)
  })
})
...
describe('regime multiplier override', () => {
  test('applyRegimeModifier uses override for counter regime', () => {
    ...
    expect(overrideResult).toBeCloseTo(conf * 0.50, 6)
  ...
  test('applyRegimeModifier ignores override for aligned regime', () => {
    ...
    const overrideResult = applyRegimeModifier(conf, 'long', 'BULL', {
      REGIME_MULT_COUNTER: 0.10,
      REGIME_MULT_NEUTRAL: 0.10,
    })
  ...
  test('undefined params = same as no params for applyRegimeModifier', () => {
    ...
    const undefinedParams = applyRegimeModifier(conf, 'long', 'BEAR', undefined)
