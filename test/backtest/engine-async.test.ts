...
describe("runBacktestAsync", () => {
  test("empty candles → empty result + done callback", async () => {
    ...
    const result = await runBacktestAsync(candles, BASE_CONFIG, (p) =>
      progress.push({ ...p }),
    );
  ...
  test("produces same metrics as sync runBacktest", async () => {
    ...
    expect(asyncResult.metrics.totalTrades).toBe(
      syncResult.metrics.totalTrades,
    );
    expect(asyncResult.metrics.netPnl).toBeCloseTo(
      syncResult.metrics.netPnl,
      4,
    );
    expect(asyncResult.metrics.winRate).toBeCloseTo(
      syncResult.metrics.winRate,
      4,
    );
  });
