...
describe('TradeSimulator', () => {
  ...
  function fillPosition(s: TradeSimulator, setup: ActiveSetup, atr = 0, mult = 2.0): void {
  ...
  test('detects SL hit for long position', () => {
    fillPosition(sim, makeSetup({ entryPrice: 100, slPrice: 95, tpPrice: 110 }))
  ...
  test('TP1 hit triggers partial close (position stays open for TP2/trail)', () => {
    fillPosition(sim, makeSetup({ entryPrice: 100, slPrice: 95, tpPrice: 110, patternData: { tp2Price: 120 } }))
  ...
  test('detects SL hit for short position', () => {
    ...
    const setup = makeSetup({ side: 'short', entryPrice: 100, slPrice: 105, tpPrice: 90 })
    sim.tryFill(setup, 0)
  ...
  test('full exit when TP1 + TP2 both hit on wide bar', () => {
    fillPosition(sim, makeSetup({
      side: 'long', entryPrice: 100, slPrice: 95, tpPrice: 110,
  ...
  test('closeAll closes remaining positions at given price', () => {
    ...
    sim.checkBar('BTC', fillBar, 1)
    // Fill ETH
    sim.tryFill(makeSetup({ id: 'ETH:1h:fvg:long', coin: 'ETH', entryPrice: 50, slPrice: 47, tpPrice: 55 }), 1)
  ...
  test('slippage and commission affect PnL on SL exit', () => {
    ...
    fillPosition(simClean, makeSetup({ entryPrice: 100, slPrice: 95, tpPrice: 110 }))
    ...
    fillPosition(simReal, makeSetup({ entryPrice: 100, slPrice: 95, tpPrice: 110 }))
