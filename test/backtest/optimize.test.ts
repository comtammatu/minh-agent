  ...
  for (const coin of coins) {
    ...
    for (const tf of tfs) {
      ...
      for (let i = 0; i < numCandles; i++) {
        ...
        candles.push(makeCandle(baseTs + i * intervalMs, price, 1000 + Math.random() * 5000))
      }
...
describe('optimize', () => {
  ...
  test('holdout validation uses separate data', () => {
    ...
    const holdoutResults = validateHoldout(holdoutCandles, baseConfig, wfConfig, [trial])
  ...
  test('current P3 config produces valid trial result', () => {
    ...
    const p3Params: StrategyParams = {
      ...
      REGIME_MULT_NEUTRAL: 0.80, // from config.ts REGIME_MULTIPLIERS.neutral
      SMC_DRILLDOWN_CONFIDENCE_BASE: 0.70,
      SL_WICK_ATR_MULT: 0.5,
      SMC_MIN_RR: 2.0,
    }
  ...
  test('selectTopN filters errors and sorts by PF', () => {
    const trials: TrialResult[] = [
      ...
      { trialIndex: 2, params: {} as StrategyParams, oosPF: 1.5, oosExpectancy: 5, maxDD: 0.5, winRate: 0.4, tradeCount: 30, numWindows: 5, durationMs: 100 },
      { trialIndex: 3, params: {} as StrategyParams, oosPF: 5.0, oosExpectancy: 25, maxDD: 0.1, winRate: 0.7, tradeCount: 2, numWindows: 5, durationMs: 100 }, // <5 trades → filtered
      { trialIndex: 4, params: {} as StrategyParams, oosPF: 4.0, oosExpectancy: 20, maxDD: 0.15, winRate: 0.65, tradeCount: 15, numWindows: 5, durationMs: 100, error: 'timeout' }, // error → filtered
    ]
  ...
  test('formatConsoleTable contains expected headers', () => {
    const mockResults: HoldoutResult[] = [
      {
        trialIndex: 0,
        params: { MIN_CONFIDENCE: 0.55, REGIME_MULT_COUNTER: 0.25 } as StrategyParams,
        oosPF: 2.5,
        oosExpectancy: 12.5,
        maxDD: 0.35,
        winRate: 0.52,
