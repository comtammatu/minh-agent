...
function makeCandle(t: number, price = 50000): Candle {
  return { t, o: price, h: price + 100, l: price - 100, c: price + 50, v: 1000 }
}
...
describe('upsertCandle (DB)', () => {
  ...
  it('inserts a new candle', async () => {
    ...
    const rows = await sql`SELECT * FROM candles WHERE coin = 'TEST' AND interval = '1m'`
...
describe('bulkUpsertCandles (DB)', () => {
  ...
  it('inserts multiple candles in one batch', async () => {
    ...
    const result = await sql`SELECT COUNT(*)::int AS cnt FROM candles WHERE coin = 'BULK'`
...
describe('store onPersist callback', () => {
  it('fires callback on appendCandle', async () => {
    ...
    const { appendCandle, setOnPersist, clearOnPersist, clearStore } = await import('../../src/feed/store.js')

    const calls: Array<{ coin: string; interval: CandleInterval; candle: Candle }> = []
  ...
  it('does not fire when no callback set', async () => {
    const { appendCandle, clearOnPersist, clearStore } = await import('../../src/feed/store.js')
