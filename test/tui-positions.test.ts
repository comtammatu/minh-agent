...
import type { ExchangePositionSnapshot, PositionState } from '../src/agent/types.js'

function baseTracked(overrides: Partial<PositionState> & Pick<PositionState, 'positionId' | 'coin'>): PositionState {
  return {
...
describe('mergeExchangeAndTrackedForTui', () => {
  it('shows HL-only positions when monitor is empty', () => {
    const exchange: ExchangePositionSnapshot[] = [
      { coin: 'BTC', size: 0.05, entryPrice: 50_000, unrealizedPnl: 10, liquidationPrice: 40_000, leverage: 10 },
    ]
  ...
  it('enriches exchange row from tracked when coin matches', () => {
    const tracked = new Map<string, PositionState>([
      ['p1', baseTracked({ positionId: 'p1', coin: 'ETH', side: 'short', slPrice: 3100, tpPrice: 2800 })],
    ])
    const exchange: ExchangePositionSnapshot[] = [
      { coin: 'ETH', size: -2, entryPrice: 3000, unrealizedPnl: -5, liquidationPrice: null, leverage: 3 },
    ]
