...
import { checkBookStaleness, getLatestBook } from "../../src/feed/orderbook.js";
...
describe("depth cap logic", () => {
  it("imbalance calculation works with capped levels", () => {
    ...
    const bids: [number, number][] = Array.from({ length: 25 }, (_, i) => [
      100 - i,
      10,
    ]);
    const asks: [number, number][] = Array.from({ length: 25 }, (_, i) => [
      101 + i,
      10,
    ]);
  ...
  it("bid-heavy after cap gives positive imbalance", () => {
    const bids: [number, number][] = Array.from({ length: 20 }, (_, i) => [
      100 - i,
      50,
    ]);
    const asks: [number, number][] = Array.from({ length: 20 }, (_, i) => [
      101 + i,
      10,
    ]);
