...
import type { Candle, DeltaState, KeyZone } from "../../src/types.js";

// ── Helpers ─────────────────────────────────────────────────────────────────

const demandZone: KeyZone = {
  type: "demand",
  top: 100,
  bottom: 95,
  strength: 0.7,
...
describe("buildVolumeProfile", () => {
  it("matches the previous naive implementation", () => {
    ...
    const naiveBuildVolumeProfile = (
      ...
      if (startIdx < 0 || endIdx >= series.length || startIdx >= endIdx)
        return null;
      ...
      for (let i = startIdx; i <= endIdx; i++) {
        ...
        const highBin = Math.min(
          numBins - 1,
          Math.floor((candle.h - lo) / binSize),
        );
      ...
      if (zoneStart !== -1) {
        lvn.push(
          bins[Math.floor((zoneStart + bins.length - 2) / 2)]?.priceLevel,
        );
      }
    ...
    expect(buildVolumeProfile(candles, 5, 95)).toEqual(
      naiveBuildVolumeProfile(candles, 5, 95),
    );
    expect(
      buildVolumeProfile(candles, 10, 110, { numBins: 32, valueAreaPct: 0.68 }),
    ).toEqual(
      naiveBuildVolumeProfile(candles, 10, 110, {
        numBins: 32,
        valueAreaPct: 0.68,
      }),
    );
...
describe("deltaConfirm", () => {
  ...
  it("strong selling at demand (divergence) → -0.10", () => {
    ...
    expect(deltaConfirm(d, demandZone)).toBe(-0.1);
  });
  ...
  it("strong buying at supply (divergence) → -0.10", () => {
    const d = makeDelta({ delta: 80, buyVol: 90, sellVol: 10 });
    expect(deltaConfirm(d, supplyZone)).toBe(-0.1);
  });
...
describe("bidAskImbalance", () => {
  ...
  it("ask-heavy → negative", () => {
    ...
    // (10 - 90) / 100 = -0.80
    expect(bidAskImbalance(bids, asks)).toBeCloseTo(-0.8, 2);
  });
...
describe("bookConfirm", () => {
  ...
  describe("demand zone", () => {
    it("absorption (imbalance >= 0.6) → +0.20", () => {
      expect(bookConfirm(0.7, demandZone)).toBe(0.2);
    });

    it("bid-heavy (imbalance >= 0.3) → +0.10", () => {
      expect(bookConfirm(0.35, demandZone)).toBe(0.1);
    });

    it("ask-heavy counter (imbalance <= -0.3) → -0.10", () => {
      expect(bookConfirm(-0.5, demandZone)).toBe(-0.1);
    });
  ...
  describe("supply zone", () => {
    it("absorption (imbalance <= -0.6) → +0.20", () => {
      expect(bookConfirm(-0.7, supplyZone)).toBe(0.2);
    });

    it("ask-heavy (imbalance <= -0.3) → +0.10", () => {
      expect(bookConfirm(-0.35, supplyZone)).toBe(0.1);
    });

    it("bid-heavy counter (imbalance >= 0.3) → -0.10", () => {
      expect(bookConfirm(0.5, supplyZone)).toBe(-0.1);
    });
...
describe("fundingConfirm", () => {
  ...
  it("negative funding + long → +0.10 (contrarian)", () => {
    expect(fundingConfirm(-0.0005, "long")).toBe(0.1);
  });

  it("positive funding + short → +0.10 (contrarian)", () => {
    expect(fundingConfirm(0.0005, "short")).toBe(0.1);
  });
...
describe("oiConfirm", () => {
  ...
  it("OI spike (>= 5%) + long → +0.10", () => {
    expect(oiConfirm(0.08, "long")).toBe(0.1);
  });

  it("OI spike (>= 5%) + short → +0.10", () => {
    expect(oiConfirm(0.06, "short")).toBe(0.1);
  });
  ...
  it("exactly at threshold → +0.10", () => {
    expect(oiConfirm(0.05, "short")).toBe(0.1);
  });
