...
function makeCandles(count: number, startTs: number = Date.UTC(2024, 0, 1)): Candle[] {
...
describe('indicator-cache', () => {
  ...
  test('reuses structure breaks array for identical coin/tf/bar snapshot', () => {
    ...
    const first = getCachedStructureBreaks('BTC', '1h', candles, idx, { tolerance: 0.1 })
    const second = getCachedStructureBreaks('BTC', '1h', candles, idx, { tolerance: 0.1 })
  ...
  test('keeps structure break caches isolated by tolerance', () => {
    ...
    const first = getCachedStructureBreaks('BTC', '1h', candles, idx, { tolerance: 0 })
    const second = getCachedStructureBreaks('BTC', '1h', candles, idx, { tolerance: 0.25 })
  ...
  test('reuses HTF structure bias for identical coin/tf/bar snapshot', () => {
    ...
    const first = getCachedHtfStructureBias('BTC', '4h', candles, idx, { swingLookback: 5, tolerance: 0.1 })
    const second = getCachedHtfStructureBias('BTC', '4h', candles, idx, { swingLookback: 5, tolerance: 0.1 })
  ...
  test('keeps Wyckoff caches isolated by params', () => {
    ...
    const first = getCachedWyckoff('BTC', '1h', candles, idx, { rangePeriod: 20, trendPeriod: 50 })
    const second = getCachedWyckoff('BTC', '1h', candles, idx, { rangePeriod: 10, trendPeriod: 50 })
  ...
  test('keeps HTF structure bias caches isolated by params', () => {
    ...
    const first = getCachedHtfStructureBias('BTC', '4h', candles, idx, { swingLookback: 5, tolerance: 0 })
    const second = getCachedHtfStructureBias('BTC', '4h', candles, idx, { swingLookback: 3, tolerance: 0 })
