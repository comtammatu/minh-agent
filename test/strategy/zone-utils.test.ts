...
function makeWaveCandles(count: number, startTs: number = Date.UTC(2024, 0, 1)): Candle[] {
...
describe('computeStructureTargets', () => {
  test('matches fallback behavior when caller provides long-side swing pivots', () => {
    ...
    const opposingZones: KeyZone[] = [
      { type: 'supply', top: entry + 8, bottom: entry + 6.5, strength: 0.8, origin: 'fvg', createdAtIdx: idx - 10 },
    ]

    const fallback = computeStructureTargets(candles, idx, entry, sl, 'long', opposingZones)
    const pivots = findPivots(candles, idx, 5)
    const injected = computeStructureTargets(candles, idx, entry, sl, 'long', opposingZones, pivots)
  ...
  test('matches fallback behavior when caller provides ATR explicitly', () => {
    ...
    const fallback = computeStructureTargets(candles, idx, entry, sl, 'long', opposingZones)
    const pivots = findPivots(candles, idx, 5)
    const injected = computeStructureTargets(candles, idx, entry, sl, 'long', opposingZones, pivots, atr(candles, idx, 14))
  ...
  test('matches fallback behavior when caller provides short-side swing pivots', () => {
    ...
    const sl = entry + 4.5
    const opposingZones: KeyZone[] = [
      { type: 'demand', top: entry - 6.5, bottom: entry - 8, strength: 0.8, origin: 'swing', createdAtIdx: idx - 12 },
    ]

    const fallback = computeStructureTargets(candles, idx, entry, sl, 'short', opposingZones)
    const pivots = findPivots(candles, idx, 5)
    const injected = computeStructureTargets(candles, idx, entry, sl, 'short', opposingZones, pivots)
