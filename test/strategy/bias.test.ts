...
describe('determineBias', () => {
  test('matches the previous fallback behavior when caller provides cached pivots', () => {
    ...
    expect(determineBias(candles, idx, htfCandles, pivots as PivotPoint[])).toEqual(naiveDetermineBias())
  })
  ...
  test('matches fallback behavior when caller provides cached breaks and htfBreaks', () => {
    ...
    const fallback = determineBias(candles, idx, htfCandles, pivots as PivotPoint[])
    const injected = determineBias(candles, idx, htfCandles, pivots as PivotPoint[], { breaks, htfBreaks })
  ...
  test('matches fallback behavior when caller provides cached Wyckoff snapshots', () => {
    ...
    const fallback = determineBias(candles, idx, htfCandles, pivots as PivotPoint[], { breaks, htfBreaks })
    const injected = determineBias(candles, idx, htfCandles, pivots as PivotPoint[], {
      breaks,
      htfBreaks,
      wyckoff,
      htfWyckoff,
