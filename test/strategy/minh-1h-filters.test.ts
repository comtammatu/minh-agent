...
import { MinhStrategy } from "../../src/strategy/minh/index.js";
...
describe("scan1hSameTF quality filters", () => {
  let strategy: MinhStrategy;

  beforeEach(() => {
    strategy = new MinhStrategy();
    strategy.clearState();
  });
  ...
  test("10. CHoCH entry: no penalty applied (confidence higher than BOS)", () => {
    ...
    strategy.clearState();
    const sigHighBase = strategy.scan(
      "BTC",
      "1h",
      CANDLES,
      SIG_IDX,
      undefined,
      { MINH_1H_CONFIDENCE_BASE: 0.8 },
    );
    // Medium base (0.60) - BOS penalty = 0.45 → needs more bonuses to pass
    strategy.clearState();
    const sigMedBase = strategy.scan("BTC", "1h", CANDLES, SIG_IDX, undefined, {
      MINH_1H_CONFIDENCE_BASE: 0.6,
    });
  ...
  test("11. BOS + low base → below MIN_CONFIDENCE → null", () => {
    ...
    const result = strategy.scan("BTC", "1h", CANDLES, SIG_IDX, undefined, {
      MINH_1H_CONFIDENCE_BASE: 0.4,
    });
    expect(result).toBeNull();
  });
  ...
  test("13. htfOpposed + CHoCH → signal (not blocked)", () => {
    ...
    strategy.clearState();
    const sigHTFOpp = strategy.scan("BTC", "1h", CANDLES, SIG_IDX, {
      htfCandles: HTF_BEARISH,
    });
  ...
  test("18. ADX < 20 → null", () => {
    ...
    for (let i = MIN_CANDLES_FOR_SCAN; i < flat.length - 1; i++) {
      if (strategy.scan("BTC", "1h", flat, i)) {
        anySignal = true;
        break;
      }
    }
  ...
  test("19. non-core coin → null (1h allowlist gate)", () => {
    const coreCoin = MINH_1H_ALLOWED_COINS[0];
