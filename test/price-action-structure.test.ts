...
import { classifySwings, detectStructuralBias } from '../src/indicators/price-action.js'
...
function buildDowntrend(count: number): Candle[] {
  const candles: Candle[] = []
  for (let i = 0; i < count; i++) {
    const base = 200 - i * 2
    if (i % 2 === 0) {
...
describe('detectStructuralBias', () => {
  it('returns neutral for fewer than 4 swings', () => {
    ...
    expect(detectStructuralBias(swings)).toEqual({ bias: 'neutral', confidence: 0 })
  })
